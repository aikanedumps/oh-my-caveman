...
`/reload-plugins` registers the freshly installed hooks; `/oh-my-caveman:setup` (alias `/setup`) runs the post-install wizard that bakes the local node binary path into hooks.json (needed for nvm/fnm users) and configures the HUD statusline. Skip these and the hook chain may not fire on first session.

After setup: open any Claude Code session. Caveman mode activates automatically.
