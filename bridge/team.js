...
function validateTmux(hasTmuxContext = false) {
  ...
  try {
    tmuxCmd(["-V"], { stripTmux: true, timeout: 5e3, stdio: "pipe" });
  } catch {
    throw new Error(
...
function listActiveSessions(teamName) {
  ...
  try {
    const output2 = tmuxCmd(["list-sessions", "-F", "#{session_name}"], {
      timeout: 5e3,
...
var init_types2 = __esm({
  "src/features/delegation-routing/types.ts"() {
    ...
    DEPRECATED_ROLE_ALIASES = {
      ...
      "harsh-critic": "critic",
      // Short user-friendly alias accepted by /team role-routing.
