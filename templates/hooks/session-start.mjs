...
async function checkForUpdates(currentVersion) {
  ...
  try {
    const response = await fetch('https://registry.npmjs.org/oh-my-caveman/latest', {
...
async function main() {
  try {
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      if ((pkg?.name === 'oh-my-caveman' || pkg?.name === 'oh-my-caveman') && pkg?.version) {
...
oh-my-caveman v${updateInfo.latestVersion} is available (current: v${updateInfo.currentVersion}).

ACTION: Use AskUserQuestion to ask the user if they want to upgrade now. Offer these options:
- "Upgrade now" (Recommended): Run \`npm install -g oh-my-caveman@latest\` via Bash, then run \`omc install --force --skip-claude-check --refresh-hooks\` to reconcile hooks and CLAUDE.md
...
A new version of oh-my-caveman is available: v${updateInfo.latestVersion} (current: ${updateInfo.currentVersion})
