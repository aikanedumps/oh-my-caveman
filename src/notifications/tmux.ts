...
import { tmuxCmd } from "../cli/tmux-utils.js";
...
export function getCurrentTmuxSession(): string | null {
  ...
  try {
    ...
    if (paneId) {
      const lines = tmuxCmd(['list-panes', '-a', '-F', '#{pane_id} #{session_name}'], {
        timeout: 3000,
    ...
    const sessionName = tmuxCmd(['display-message', '-p', '#S'], {
      timeout: 3000,
      stdio: ["pipe", "pipe", "pipe"],
    }).trim();
...
export function getCurrentTmuxPaneId(): string | null {
  ...
  try {
    const paneId = tmuxCmd(['display-message', '-p', '#{pane_id}'], {
      timeout: 3000,
      stdio: ["pipe", "pipe", "pipe"],
    }).trim();
