...
describe('hasEnabledOmcPlugin', () => {
  ...
  describe('Modern Claude Code 1.x format (`enabledPlugins`)', () => {
    it('returns true when enabledPlugins.oh-my-caveman@omc is true', async () => {
      writeSettings({
        enabledPlugins: {
          'oh-my-caveman@omc': true,
    ...
    it('returns true for any pluginId substring matching oh-my-caveman', async () => {
      writeSettings({
        enabledPlugins: {
          'oh-my-caveman-fork@somerepo': true,
        },
      });
      const { hasEnabledOmcPlugin } = await freshInstaller();
      expect(hasEnabledOmcPlugin()).toBe(true);
    ...
    it('returns false when enabledPlugins.oh-my-caveman@omc is explicitly false', async () => {
      writeSettings({
        enabledPlugins: {
          'oh-my-caveman@omc': false,
        },
      });
      const { hasEnabledOmcPlugin } = await freshInstaller();
    ...
    it('returns false when enabledPlugins has no oh-my-caveman entry', async () => {
      writeSettings({
        enabledPlugins: {
          'unrelated-plugin@foo': true,
        },
    ...
    it('handles enabledPlugins as an array of plugin id strings', async () => {
      writeSettings({
        enabledPlugins: ['oh-my-caveman@omc', 'other'],
      });
      const { hasEnabledOmcPlugin } = await freshInstaller();
      expect(hasEnabledOmcPlugin()).toBe(true);
    });
  ...
  describe('Legacy `plugins` field (backward compatibility)', () => {
    it('returns true when plugins.oh-my-caveman@omc is true', async () => {
      writeSettings({
        plugins: {
          'oh-my-caveman@omc': true,
        },
    ...
    it('returns true when plugins is an array with oh-my-caveman entry', async () => {
      writeSettings({
        plugins: ['oh-my-caveman'],
      });
      const { hasEnabledOmcPlugin } = await freshInstaller();
      expect(hasEnabledOmcPlugin()).toBe(true);
    });
  ...
  describe('Mixed format support', () => {
    ...
    it('returns true when modern enabledPlugins has the entry but legacy plugins does not', async () => {
      writeSettings({
        enabledPlugins: { 'oh-my-caveman@omc': true },
