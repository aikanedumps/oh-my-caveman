...
// NOTE (US-101 / SSRF DNS rebinding): All HTTP requests in this module run via
// either `fetch()` (detectFromApi) or `execFileSync('curl', ...)` (REST fallbacks).
// Neither uses Node's `https.request`, so the `{lookup: validatedLookup}` rebinding
// defense in usage-api.ts does not apply here. Hostname-level validation via
// `validateUrlForSSRF` (called once in `validateGiteaUrl`) is the strongest
// guard available without rewriting these to use `https.request` directly.
// If/when this module migrates to `https.request`, wire `validatedLookup` in.
