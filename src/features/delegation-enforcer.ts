...
function canonicalizeSubagentType(subagentType: string): string {
  const hasPrefix = subagentType.startsWith('oh-my-caveman:');
  const rawAgentType = subagentType.replace(/^oh-my-caveman:/, '');
  const canonicalAgentType = normalizeDelegationRole(rawAgentType);
  return hasPrefix ? `oh-my-caveman:${canonicalAgentType}` : canonicalAgentType;
}
...
export function enforceModel(agentInput: AgentInput): EnforcementResult {
  ...
  const agentType = canonicalSubagentType.replace(/^oh-my-caveman:/, '');
...
export function getModelForAgent(agentType: string): string {
  const normalizedType = normalizeDelegationRole(agentType.replace(/^oh-my-caveman:/, ''));
