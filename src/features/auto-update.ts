 ...
 * Auto-Update System
 *
 * Provides version checking and auto-update functionality for oh-my-caveman.
 *
 * Features:
...
export const REPO_OWNER = 'aikanedumps';
export const REPO_NAME = 'oh-my-caveman';
 ...
 * The marketplace clone at ~/.claude/plugins/marketplaces/oh-my-caveman/ is used by
...
function syncMarketplaceClone(verbose: boolean = false): { ok: boolean; message: string } {
  const marketplacePath = join(getClaudeConfigDir(), 'plugins', 'marketplaces', 'oh-my-caveman');
...
export function syncPluginCache(verbose: boolean = false): { synced: boolean; skipped: boolean; errors: string[] } {
  ...
  const pluginCacheRoot = join(configDir, 'plugins', 'cache', 'oh-my-caveman', 'oh-my-caveman');
  ...
  try {
    ...
    const sourceRoot = join(npmRoot, 'oh-my-caveman');
...
export function getInstalledVersion(): VersionMetadata | null {
  if (!existsSync(VERSION_FILE)) {
    ...
    try {
      ...
      const result = execSync('npm list -g oh-my-caveman --json', {
        encoding: 'utf-8',
        timeout: 5000,
        stdio: 'pipe'
      });
      const data = JSON.parse(result);
      if (data.dependencies?.['oh-my-caveman']?.version) {
        return {
          version: data.dependencies['oh-my-caveman'].version,
  ...
  try {
    ...
    if (shouldBlockStandaloneUpdateInCurrentSession() && !options?.standalone) {
      return {
        ...
        previousVersion,
        newVersion: 'unknown',
        message: 'Running inside an active Claude Code plugin session. Use "/plugin install oh-my-caveman" to update, or pass --standalone to force npm update.',
      };
    }
    ...
    try {
      execSync('npm install -g oh-my-caveman@latest', {
        encoding: 'utf-8',
      ...
      throw new Error(
        ...
        '  npm install -g oh-my-caveman@latest\n' +
        'Or use: /plugin install oh-my-caveman\n' +
...
export function formatUpdateNotification(checkResult: UpdateCheckResult): string {
  if (!checkResult.updateAvailable) {
    return `oh-my-caveman is up to date (v${checkResult.currentVersion ?? 'unknown'})`;
  }

  const lines = [
    '╔═══════════════════════════════════════════════════════════╗',
    '║           oh-my-caveman Update Available!              ║',
    '╚═══════════════════════════════════════════════════════════╝',
    '',
    ...
    '  Or reinstall via: /plugin install oh-my-caveman',
    ''
  ];
