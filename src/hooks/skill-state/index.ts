 ...
 *
 * Fix for: https://github.com/aikanedumps/oh-my-caveman/issues/1033
 */
 ...
 * @param rawSkillName - The original skill name as invoked (e.g., 'oh-my-caveman:plan'
 *   or 'plan'). When provided, only skills invoked with the 'oh-my-caveman:' prefix
 ...
 *   See: https://github.com/aikanedumps/oh-my-caveman/issues/1581
 */
export function getSkillProtection(skillName: string, rawSkillName?: string): SkillProtectionLevel {
  ...
  if (rawSkillName != null && !rawSkillName.toLowerCase().startsWith('oh-my-caveman:')) {
    return 'none';
  }
  const normalized = skillName.toLowerCase().replace(/^oh-my-caveman:/, '');
