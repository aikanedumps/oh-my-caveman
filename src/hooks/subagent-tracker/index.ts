...
export function getAgentDashboard(directory: string): string {
  ...
  for (const agent of running) {
    ...
    const shortType = agent.agent_type.replace("oh-my-caveman:", "");
...
export function getAgentObservatory(directory: string): {
  ...
  for (const intervention of interventions.slice(0, 3)) {
    const shortType = intervention.agent_type.replace("oh-my-caveman:", "");
    ...
    if (agents.length > 1) {
      ...
      for (let i = 1; i < agents.length; i++) {
        interventions.push({
          ...
          reason: `File conflict on ${file} with ${agents[0].type.replace("oh-my-caveman:", "")}`,
          suggested_action: "warn",
          auto_execute: false,
        });
      }
...
export function detectFileConflicts(directory: string): Array<{
  ...
  for (const agent of running) {
    for (const file of agent.file_ownership || []) {
      ...
      fileToAgents
        .get(file)!
        .push(agent.agent_type.replace("oh-my-caveman:", ""));
    }
  }
