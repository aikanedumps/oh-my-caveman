/**
 * Ralph Hook — self-referential work loop with PRD support and architect verification.
 * Continues until cancelled via /oh-my-caveman:cancel or all PRD stories complete.
 */
...
/**
 * Sentinel returned by incrementRalphIteration when the file lock could not
 * be acquired within the timeout (transient contention, safe to retry).
 * Callers should treat this as a retryable signal, NOT as a silent no-op.
 */
export const RALPH_LOCK_BUSY: unique symbol = Symbol("RALPH_LOCK_BUSY");
 ...
 *
 * The read-modify-write is protected by an advisory file lock to prevent
 * lost updates when multiple stop-hook invocations race (e.g. tool-use
 * callbacks arriving concurrently).
 *
 * @returns Updated state on success, RALPH_LOCK_BUSY if the lock could not
 *   be acquired within the timeout (transient — caller should retry), or
 *   null if ralph is not active.
 */
export function incrementRalphIteration(
  directory: string,
  sessionId?: string,
): RalphLoopState | null | typeof RALPH_LOCK_BUSY {
  // Resolve the exact state file path so the lock is adjacent to the file
  // being written by writeRalphState → writeModeState → resolveFile.
  const statePath = sessionId
    ? resolveSessionStatePath("ralph", sessionId, directory)
    : resolveStatePath("ralph", directory);
  const lockPath = lockPathFor(statePath);

  const handle = acquireFileLockSync(lockPath, { timeoutMs: 500 });
  if (!handle) {
    console.warn(
      `[ralph] incrementRalphIteration: could not acquire file lock within 500ms (${lockPath}). Signalling lock-busy for retry.`,
    );
    return RALPH_LOCK_BUSY;
  }
...
export function createRalphLoopHook(directory: string): RalphLoopHook {
  const startLoop = (
    ...
    if (isUltraQAActive(directory, sessionId)) {
      console.error(
        "Cannot start Ralph Loop while UltraQA is active. Cancel UltraQA first with /oh-my-caveman:cancel.",
      );
      return false;
    }
