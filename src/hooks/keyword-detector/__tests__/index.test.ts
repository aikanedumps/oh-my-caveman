...
Summarize the failure mode only.`);
      ...
      expect(result).not.toContain('Skill: oh-my-caveman:ralph');
...
Why did this happen?`);
      ...
      expect(result).not.toContain('Skill: oh-my-caveman:autopilot');
    });
...
This article argues that fake popularity signals damage trust in open source.`;
  ...
  describe('intent-pattern guards: file paths and code blocks (spec h)', () => {
    ...
    it('inline backtick `/oh-my-caveman:ralph` does NOT detect ralph', () => {
      const result = detectKeywordsWithType('run `/oh-my-caveman:ralph` if needed');
      expect(result.find((r) => r.type === 'ralph')).toBeUndefined();
    });
  ...
  // -------------------------------------------------------------------------
  // Unified prefix detector (spec g) — /skill, /omc:skill, /oh-my-caveman:skill
  // all seed the same canonical state (T3 implementation required)
  // -------------------------------------------------------------------------

  describe('unified prefix detector: /omc: and /oh-my-caveman: forms (spec g)', () => {
    it('/omc:ralph fix auth detects ralph', () => {
      const result = detectKeywordsWithType('/omc:ralph fix auth');
      expect(result.find((r) => r.type === 'ralph')).toBeDefined();
    });

    it('/oh-my-caveman:ralph fix auth detects ralph', () => {
      const result = detectKeywordsWithType('/oh-my-caveman:ralph fix auth');
      expect(result.find((r) => r.type === 'ralph')).toBeDefined();
    });
  ...
  describe('parseExplicitWorkflowSlashInvocation — parser unit tests (spec g)', () => {
    ...
    it('parses /oh-my-caveman:ralph and normalizes skill name', () => {
      const result = parseExplicitWorkflowSlashInvocation('/oh-my-caveman:ralph debug this');
      expect(result).not.toBeNull();
      expect(result!.skill).toBe('ralph');
    });
    ...
    it('all three prefix forms produce the same skill name for autopilot', () => {
      ...
      const full = parseExplicitWorkflowSlashInvocation('/oh-my-caveman:autopilot go');
