...
import { validateAnthropicBaseUrl, validatedLookup } from '../utils/ssrf-guard.js';
import {
...
function getLegacyCachePath(): string {
  return join(getClaudeConfigDir(), 'plugins', 'oh-my-caveman', '.usage-cache.json');
}
...
function getCachePath(source: 'anthropic' | 'zai' | 'minimax'): string {
  return join(getClaudeConfigDir(), 'plugins', 'oh-my-caveman', `.usage-cache-${source}.json`);
}
