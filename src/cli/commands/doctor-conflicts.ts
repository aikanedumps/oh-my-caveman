...
export function formatReport(report: ConflictReport, json: boolean): string {
  ...
  lines.push('');
  lines.push(colors.bold('🔍 oh-my-caveman Conflict Diagnostic'));
  ...
  if (report.claudeMdStatus) {
    ...
    if (report.claudeMdStatus.hasMarkers) {
      ...
      lines.push(`    ${colors.gray('Run /oh-my-caveman:omc-setup to add markers')}`);
      if (report.claudeMdStatus.hasUserContent) {
  ...
  if (report.hasConflicts) {
    ...
    lines.push(`${colors.gray('Review the issues above and run /oh-my-caveman:omc-setup if needed')}`);
  } else {
