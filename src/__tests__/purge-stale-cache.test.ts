...
describe('purgeStalePluginCacheVersions', () => {
  ...
  it('handles multiple marketplaces and plugins', () => {
    ...
    const active2 = join(cacheDir, 'omc/oh-my-caveman/4.3.0');
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      ...
      plugins: {
        ...
        'oh-my-caveman@omc': [{ installPath: active2 }],
      },
    }));
    ...
    mockedReaddirSync.mockImplementation((p, _opts?) => {
      ...
      if (ps.endsWith('omc')) return [dirent('oh-my-caveman')] as any;
      if (ps.endsWith('oh-my-caveman')) return [dirent('4.3.0')] as any;
      return [] as any;
    });
  ...
  it('does nothing when all cache versions are active', () => {
    const cacheDir = '/mock/.claude/plugins/cache';
    const active = join(cacheDir, 'omc/oh-my-caveman/4.3.0');
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      version: 2,
      plugins: {
        'oh-my-caveman@omc': [{ installPath: active }],
      },
    }));
  ...
  it('replaces stale version dir with symlink to active version in same namespace', () => {
    ...
    const cacheDir = '/mock/.claude/plugins/cache';
    const activeVersion = join(cacheDir, 'omc/oh-my-caveman/4.14.5');
    const staleVersion = join(cacheDir, 'omc/oh-my-caveman/4.14.4');
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      version: 2,
      plugins: {
        'oh-my-caveman@omc': [{ installPath: activeVersion, version: '4.14.5' }],
      },
    }));
    ...
    mockedReaddirSync.mockImplementation((p, _opts?) => {
      ...
      if (ps === cacheDir) return [dirent('omc')] as any;
      if (ps.endsWith('omc')) return [dirent('oh-my-caveman')] as any;
      if (ps.endsWith('oh-my-caveman')) return [dirent('4.14.4'), dirent('4.14.5')] as any;
      return [] as any;
    });
  ...
  it('skips version directory entries where isDirectory() returns false (existing symlinks)', () => {
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      version: 2,
      plugins: {
        'oh-my-caveman@omc': [{ installPath: activeVersion }],
      },
    }));
