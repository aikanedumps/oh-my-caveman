...
describe('delegation-enforcer', () => {
  ...
  describe('enforceModel', () => {
    ...
    it('rewrites deprecated aliases to canonical agent names before injecting model', () => {
      const input: AgentInput = {
        description: 'Test task',
        prompt: 'Do something',
        subagent_type: 'oh-my-caveman:build-fixer'
      };

      const result = enforceModel(input);

      expect(result.injected).toBe(true);
      expect(result.modifiedInput.subagent_type).toBe('oh-my-caveman:debugger');
      expect(result.modifiedInput.model).toBe('sonnet');
    });
  ...
  describe('getModelForAgent', () => {
    it('returns correct model for agent with prefix', () => {
      expect(getModelForAgent('oh-my-caveman:executor')).toBe('sonnet');
      expect(getModelForAgent('oh-my-caveman:debugger')).toBe('sonnet');
      expect(getModelForAgent('oh-my-caveman:architect')).toBe('opus');
    });
