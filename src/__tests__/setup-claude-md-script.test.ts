...
`);
  ...
  it('refuses to install a canonical source that lacks OMC markers', () => {
    const fixture = createPluginFixture(`# oh-my-caveman (OMC) v9.9.9 Summary
...
`);
    ...
    writeFileSync(join(configDir, 'settings.json'), JSON.stringify({ plugins: ['oh-my-caveman'] }));
...
describe('setup-claude-md.sh stale CLAUDE_PLUGIN_ROOT resolution', () => {
  it('does not prefer a newer cache directory when it is missing required plugin assets', () => {
    ...
    const cacheBase = join(root, '.claude', 'plugins', 'cache', 'omc', 'oh-my-caveman');
    ...
    writeFileSync(join(homeRoot, '.claude', 'settings.json'), JSON.stringify({ plugins: ['oh-my-caveman'] }));
  ...
  it('prefers newer cache version when installed_plugins.json points to an existing but stale older version', () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ plugins: ['oh-my-caveman'] }),
    );
