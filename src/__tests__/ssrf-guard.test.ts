...
describe('validatedLookup (DNS rebinding defense)', () => {
  ...
  function mockDnsLookup(addresses: dns.LookupAddress[] | { err: NodeJS.ErrnoException }) {
  ...
  it('allows fully-public resolution (single address)', async () => {
    ...
    const result = await new Promise<{ err: NodeJS.ErrnoException | null; addr?: unknown; family?: number }>(
      (resolve) => {
        validatedLookup('example.com', { all: false }, (err, addr, family) =>
  ...
  it('returns array form when options.all is true', async () => {
    ...
    mockDnsLookup(addrs);
    const result = await new Promise<{ err: NodeJS.ErrnoException | null; addr?: unknown }>(
      (resolve) => {
  ...
  it('propagates dns.lookup errors (e.g. ENOTFOUND)', async () => {
    const dnsErr: NodeJS.ErrnoException = new Error('lookup failed');
