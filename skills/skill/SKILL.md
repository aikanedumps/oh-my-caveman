...
Meta-skill for managing oh-my-caveman skills via CLI-like commands.
...
**Behavior:**
1. Scan bundled built-in skills in plugin `skills/` directory (read-only)
...
```
BUILT-IN SKILLS (bundled with oh-my-caveman):
...
**Fallback:** Quality/usage stats unavailable → show "N/A"

**Built-in skill note:** Built-in skills bundled with oh-my-caveman. Discoverable/readable, not removable or editable via `/skill remove` or `/skill edit`.

---

### /skill add [name]

Interactive wizard for creating new skill.
...
```
/oh-my-caveman:<name> example-arg
```
...
2. **If found:**
   ...
   - **Ask confirmation:** "Delete '<name>' skill from <scope>? (yes/no)"
3. **If confirmed:**
...
Edit existing skill interactively.
...
Show detailed information about skill.
...
Check if skill directories exist, create if needed:
...
Scan both directories, show comprehensive inventory:
...
After scanning, use AskUserQuestion tool to offer these options:

**Question:** "What would you like to do with your local skills?"

**Options:**
1. **Add new skill** - Start skill creation wizard (invoke `/skill add`)
...
4. **Import skill** - Import skill from URL or paste content
5. **Done** - Exit wizard

**Option 3: Scan Conversation for Patterns**

Analyze current conversation context for potential skill-worthy patterns. Look for:
- Recent debugging sessions with non-obvious solutions
- Tricky bugs requiring investigation
- Codebase-specific workarounds discovered
- Error patterns that took time to resolve

Report findings, ask if user wants to extract any as skills (invoke `/learner` if yes).

**Option 4: Import Skill**

Ask user to provide either:
- **URL**: Download skill from URL (e.g., GitHub gist)
...
Validate skill format and save to chosen location.
...
**Behavior:**
Run scan from Step 2 of `/skill setup` without interactive wizard.
...
Invoked with argument → skip interactive wizard:

- `/oh-my-caveman:skill list` - Show detailed skill inventory
- `/oh-my-caveman:skill add` - Start skill creation (invoke learner)
- `/oh-my-caveman:skill scan` - Scan both skill directories

### Interactive Mode

Invoked without arguments → run full guided wizard.

---

## Benefits of Local Skills

**Automatic Application**: Claude detects triggers, applies skills automatically — no need to remember or search for solutions.

**Version Control**: Project-level skills (.omc/skills/) committed with code. Whole team benefits.

**Evolving Knowledge**: Skills improve over time as better approaches discovered, triggers refined.

**Reduced Token Usage**: Known patterns applied efficiently instead of re-solving same problems.
...
- `/oh-my-caveman:learner` - Extract skill from current conversation
- `/oh-my-caveman:note` - Save quick notes (less formal than skills)
- `/oh-my-caveman:deepinit` - Generate AGENTS.md codebase hierarchy
...
- Run `/oh-my-caveman:skill list` periodically to review skill library
- After solving tricky bug, immediately run learner to capture it
