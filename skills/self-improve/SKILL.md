...
**Loop controller** for self-improvement system. Manages full lifecycle: setup, research, planning, execution, tournament selection, history recording, visualization, and stop-condition evaluation. Delegates to specialized OMC agents and coordinates inputs and outputs.

---

## Autonomous Execution Policy

**NEVER stop or pause to ask user during improvement loop.** Once gate check passes and loop begins, run fully autonomous until stop condition met.

- **Do not ask for confirmation** between iterations or between steps within iteration.
- **Do not summarize and wait** — execute next step immediately.
- **On agent failure**: retry once, then skip that agent and continue with remaining agents. Log failure in iteration history.
- **On all plans rejected**: log it, continue to next iteration automatically.
- **On all executors failing**: log it, continue to next iteration automatically.
- **On benchmark errors**: log error, mark executor as failed, continue with other executors.
- **Only things that stop loop** are stop conditions in Step 11.
- **Trust boundary**: Loop runs benchmark commands as-is inside target repo. User explicitly confirms repo path and benchmark command during setup. Loop does NOT install packages, modify system config, or access network resources beyond what benchmark command does.
- **Sealed files**: validate.sh enforces that benchmark code cannot be modified by loop, preventing self-modification of evaluation.
...
| Planning | Hypothesis → structured plan | oh-my-caveman:planner | opus |
| Architecture Review | 6-point plan review | oh-my-caveman:architect | opus |
| Critic Review | Harness rule enforcement | oh-my-caveman:critic | opus |
| Execution | Implement plan + run benchmark | oh-my-caveman:executor | opus |
| Git Operations | Atomic merge/tag/PR | oh-my-caveman:git-master | sonnet |
| Goal Setup | Interactive interview | (directly in this skill) | N/A |
| Benchmark Setup | Create + validate benchmark | custom agent | opus |

**Research prompt**: Read `si-researcher.md` from this skill directory and pass its content as agent prompt.

**Benchmark builder**: Read `si-benchmark-builder.md` from this skill directory and pass its content as agent prompt.

**Goal clarifier**: Read `si-goal-clarifier.md` from this skill directory and execute interview directly (interactive, needs user).

---

## Inputs

Read these files at startup and at beginning of each iteration:
...
1. Check if target repo path exists. Not configured → ask user for path to repository to improve.
...
4. **Trust confirmation** (mandatory, cannot be skipped):
   a. `trust_confirmed` already `true` in agent-settings.json → skip to step 5 (resume path).
   b. Display target repo path and ask user to confirm:
      `"Self-improve will run benchmark commands inside {repo_path}. This executes arbitrary code in that repository. Confirm? [yes/no]"`
   c. User declines → abort setup and exit. Do NOT proceed.
   d. Record consent: set `trust_confirmed: true` in agent-settings.json.
5. Goal not set → read `si-goal-clarifier.md` from this skill directory and run 4-dimension Socratic interview directly in this context (Objective, Metric, Target, Scope). Write result to `.omc/self-improve/config/goal.md`.
6. Benchmark not set → read `si-benchmark-builder.md` from this skill directory, spawn custom Agent(model=opus) with its content as prompt. Agent surveys repo, creates or wraps benchmark, validates 3x, and records baseline.
   After benchmark set, confirm benchmark command with user:
   `"Benchmark command: {benchmark_command}. This will be run repeatedly during loop. Confirm? [yes/no]"`
   User declines → abort setup and exit.
7. Harness not set → confirm default harness rules (H001/H002/H003) with user or customize.
...
9. **Create improvement branch** (if it does not exist):
   ...
   ```
   Where `{goal_slug}` is derived from goal objective (lowercase, underscored). Branch already exists → skip creation. Persist `goal_slug` in agent-settings.json.
...
All git operations happen inside target repo, NOT in OMC project root.
...
- **Worktree setup** (SKILL.md creates before each executor):
  ...
  ```
- **Winner merges** via `oh-my-caveman:git-master`:
  ```
...
**Gate**: All settings must be true. Once gate passes, execute continuously without stopping.
...
**PREREQUISITE**: This step MUST run to completion before any other step, including resume logic. Idempotent and safe to run multiple times.

1. List all worktrees in target repo: `git -C {repo_path} worktree list`
2. Any worktree matching `worktrees/round_*` that does NOT belong to current iteration: remove with `git -C {repo_path} worktree remove {path} --force`
3. Run `git -C {repo_path} worktree prune` to clean up stale references
4. Handles crash recovery — orphaned worktrees from interrupted iterations cleaned before new iteration starts
...
If state cleared (cancel invoked) OR status is `user_stopped`:
  ...
  c. Clean up any active worktrees for current round (Step 0 logic)
...
Read `.omc/self-improve/config/idea.md`. Non-empty → snapshot contents for planners. Clear after planners consume.

### Step 4 — Research

Spawn 1 general-purpose Agent(model=opus) with content of `si-researcher.md` as prompt.
...
Researcher fails → proceed with history only.

### Step 5 — Plan

Spawn N `oh-my-caveman:planner`(model=opus) agents in parallel (N = `number_of_agents` from settings).
...
**6a. Architecture Review**: Spawn `oh-my-caveman:architect` with plan + 6-point checklist:
1. Testability — is hypothesis testable?
...
**6b. Critic Review**: Spawn `oh-my-caveman:critic` with plan + harness rules:
...
Critic sets `critic_approved: true` or `false`. Plans with `false` excluded from execution.

All plans rejected → log and skip to Step 9.

### Step 7 — Execute

For each approved plan, spawn `oh-my-caveman:executor`(model=opus) in parallel.
...
- **Override instructions**: Implement plan faithfully, run validate.sh before benchmarking, run benchmark command, produce Benchmark Result JSON as output.
...
2. **Filter** to `status: "success"` only. Zero candidates → skip to Step 9 (Record & Visualize).
...
4. **Ranked-candidate loop** — for each candidate in rank order (best first):
   ...
   b. **Merge** via `oh-my-caveman:git-master`: `git merge experiment/round_{n}_executor_{id} --no-ff -m "Iteration {n}: {hypothesis} (score: {before} → {after})"`
   c. **Re-benchmark** on merged state to confirm improvement
   d. Re-benchmark **confirms** improvement: **accept winner**, break loop
   e. Re-benchmark shows **regression**: **revert merge** via `git -C {repo_path} reset --hard HEAD~1`, continue to next candidate
   f. Merge **conflicts**: `git -C {repo_path} merge --abort`, continue to next candidate
5. Winner accepted AND `auto_push` is `true` in settings: **Push** improvement branch: `git -C {repo_path} push origin improve/{goal_slug}` (non-blocking).
   `auto_push` is `false` (default): skip push. Log: `"Push skipped (auto_push: false). Run manually: git -C {repo_path} push origin improve/{goal_slug}"`
6. **Archive** all non-winner branches via git-master: tag + delete
7. No candidate survived loop: no merge this round. Improvement branch stays at prior state.
...
2. Update `.omc/self-improve/state/agent-settings.json`:
   ...
   - Winner AND improvement exceeds `plateau_threshold` (`abs(new_score - best_score) >= plateau_threshold`): update `best_score`, reset `plateau_consecutive_count = 0`, reset `circuit_breaker_count = 0`
   - Winner AND improvement below threshold (`abs(new_score - best_score) < plateau_threshold`): update `best_score` if better, increment `plateau_consecutive_count += 1`, reset `circuit_breaker_count = 0`
   - No winner (all rejected, all failed, or all regressed): increment `circuit_breaker_count += 1` (do NOT increment `plateau_consecutive_count` — plateau tracks stagnating wins, not failures)
...
Evaluate ALL conditions. If ANY true, exit:
...
No stop condition → immediately go back to Step 1.
...
On invocation, before entering loop:

1. **Always run Step 0** (stale worktree cleanup) — even on fresh start
2. Read `.omc/self-improve/state/agent-settings.json`:
   - `status: "user_stopped"`: ask user `"Previous run was stopped at iteration {N}. Resume? [yes/no]"`. No → exit. Yes → continue.
   - `status: "running"`: session crashed — resume automatically (no user prompt)
...
2. `target_reached` AND `auto_pr` is `true` in settings: spawn git-master to create PR from `improve/{goal_slug}` to upstream.
   `auto_pr` is `false` (default): skip PR creation. Log: `"PR creation skipped (auto_pr: false). Run manually: gh pr create --head improve/{goal_slug} --base {target_branch}"`
...
4. Print summary report:
   ...
   ```
5. Run `/oh-my-caveman:cancel` for clean state cleanup
...
| Situation | Action |
|---|---|
| Agent fails to produce output | Retry once. Still no output → log and continue. |
