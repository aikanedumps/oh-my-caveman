...
Detect which provider user wants based on request or argument:
- Trigger/argument contains "telegram" → follow **Telegram** section
- Trigger/argument contains "discord" → follow **Discord** section
- Trigger/argument contains "slack" → follow **Slack** section
- No provider specified → use AskUserQuestion:
...
Interactive, natural-language configuration skill. Walk user through setup by asking questions with AskUserQuestion. Write result to `${CLAUDE_CONFIG_DIR:-~/.claude}/.omc-config.json`.
...
If existing config found, show user what's currently configured and ask if they want to update or reconfigure.

### Step 2: Create a Telegram Bot

Guide user through creating bot if they don't have one:
...
User types token in "Other" field.

**Validate** token:
- Must match pattern: `digits:alphanumeric` (e.g., `123456789:ABCdefGHI...`)
- If invalid, explain format and ask again
...
User types chat ID in "Other" field.

**Validate** chat ID:
- Must be number (positive for personal, negative for groups)
- If invalid, offer to help find it:
...
1. **Session end (Recommended)** - When Claude session finishes
2. **Input needed** - When Claude waiting for your response (great for long-running tasks)
3. **Session start** - When new session begins
4. **Session continuing** - When persistent mode keeps session alive

Default selection: session-end + ask-user-question.

### Step 7: Write Configuration

Read existing config, merge new Telegram settings, write back:
...
After writing config, offer to send test notification:
...
**Options:**
1. **Yes, test now (Recommended)** - Send test message to your Telegram chat
...
- **400 Bad Request: chat not found**: Chat ID wrong, or user hasn't sent `/start` to bot
...
To reconfigure: /oh-my-caveman:configure-notifications telegram
To configure Discord: /oh-my-caveman:configure-notifications discord
To configure Slack: /oh-my-caveman:configure-notifications slack
```

### Environment Variable Alternative

Skip wizard entirely by setting env vars in shell profile:
...
Env vars auto-detected by notification system without needing `.omc-config.json`.
...
**Options:**
1. **Webhook (Recommended)** - Create webhook in your Discord channel. Simple, no bot needed. Paste URL.
2. **Bot API** - Use Discord bot token + channel ID. More flexible, requires bot application.
...
User types webhook URL in "Other" field.
...
**Options:**
1. **Yes, mention a user** - Tag specific user by their Discord user ID
2. **Yes, mention a role** - Tag role by its role ID
3. **No mentions** - Post message without pinging anyone
...
Mention format: `<@USER_ID>` (e.g., `<@1465264645320474637>`)
...
Mention format: `<@&ROLE_ID>` (e.g., `<@&123456789>`)
...
**Question:** "Custom bot display name? (Shows as webhook sender name in Discord)"

**Options:**
1. **OMC (default)** - Display as "OMC"
2. **Claude Code** - Display as "Claude Code"
3. **Custom** - Enter custom name

### Step 7: Write Configuration

Read existing config, merge new Discord settings, write back:
...
Build notifications object with collected values and merge into `.omc-config.json` using jq:
...
**Options:**
1. **Yes, test now (Recommended)** - Send test message to your Discord channel
2. **No, I'll test later** - Skip testing
...
Report success or failure. If fails, help user debug (check URL, permissions, etc.).
...
To reconfigure: /oh-my-caveman:configure-notifications discord
To configure Telegram: /oh-my-caveman:configure-notifications telegram
To configure Slack: /oh-my-caveman:configure-notifications slack
```
...
Guide user through creating webhook if they don't have one:
...
**Options:**
1. **Yes, mention a user** - Tag specific user by their Slack member ID
2. **Yes, mention a channel** - Use @channel to notify everyone in channel
3. **Yes, mention @here** - Notify only active members in channel
4. **No mentions** - Post message without pinging anyone

#### If user wants to mention a user:

Ask: "What is the Slack member ID to mention? (Click on user's profile > More (⋯) > Copy member ID)"

Mention format: `<@MEMBER_ID>` (e.g., `<@U1234567890>`)
...
**Question:** "Override default notification channel? (Webhook already has default channel)"

**Options:**
1. **Use webhook default (Recommended)** - Post to channel selected during webhook setup
2. **Override channel** - Specify different channel (e.g., #alerts)

If override, ask for channel name (e.g., `#alerts`).

### Step 7: Optional Username Override

Use AskUserQuestion:

**Question:** "Custom bot display name? (Shows as webhook sender name in Slack)"
...
Read existing config, merge new Slack settings, write back:
...
**Options:**
1. **Yes, test now (Recommended)** - Send test message to your Slack channel
2. **No, I'll test later** - Skip testing
...
- **channel_not_found**: Channel override invalid
...
To reconfigure: /oh-my-caveman:configure-notifications slack
To configure Discord: /oh-my-caveman:configure-notifications discord
To configure Telegram: /oh-my-caveman:configure-notifications telegram
```
...
Without these flags, configured platforms remain dormant. Prevents unwanted notifications during development while keeping configuration persistent.
...
Trigger/argument contains "hook", "template", or "customize messages" → follow this section.

### Step 1: Detect Existing Hook Config

Check if `${CLAUDE_CONFIG_DIR:-~/.claude}/omc_config.hook.json` exists. If yes, show current configuration. If not, explain what it does.
...
**Options:**
1. **session-end** - When Claude session finishes (most common)
2. **ask-user-question** - When Claude waiting for input
3. **session-idle** - When Claude finishes and waits for input
4. **session-start** - When new session begins

### Step 3: Show Available Variables

Display template variables available for chosen event:
...
**Options:**
1. **Use default template** - Keep built-in message format
2. **Simple summary** - Short one-line format
3. **Custom** - Enter your own template

If "Simple summary", use pre-built compact template:
...
**Options:**
1. **No, same for all (Recommended)** - Use same template everywhere
...
Read or create `${CLAUDE_CONFIG_DIR:-~/.claude}/omc_config.hook.json` and merge new settings:
...
Validate template using `validateTemplate()` to check for unknown variables. If any found, warn user and offer to correct.

Offer to send test notification with new template.
...
- `/oh-my-caveman:configure-openclaw` — Configure OpenClaw gateway integration

---

## Custom Integration (OpenClaw, n8n, CLI, etc.)

Configure custom webhooks and CLI commands for services beyond native Discord/Telegram/Slack integrations.

### Routing

User says "custom integration", "openclaw", "n8n", "webhook", "cli command", or similar → follow this section.
