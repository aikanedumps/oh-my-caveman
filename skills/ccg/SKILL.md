...
CCG routes through canonical `/ask` skill (`/ask codex` + `/ask gemini`), then Claude synthesizes both outputs into one answer.

Use when parallel external perspectives needed without launching tmux team workers.
...
- Either CLI unavailable → continue with available provider, note limitation
...
> **Note:** Skill nesting (invoking skill from within active skill) not supported in Claude Code. Always use direct CLI path via Bash tool.
...
- Fall back to Claude-only answer, state CCG external advisors were unavailable

## Invocation

```bash
/oh-my-caveman:ccg <task description>
```

Example:

```bash
/oh-my-caveman:ccg Review this PR - architecture/security via Codex and UX/readability via Gemini
```
