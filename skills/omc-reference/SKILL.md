...
Use this built-in reference when detailed OMC catalog info is needed but shouldn't live in every `CLAUDE.md` session.
...
Invoke built-in workflows via `/oh-my-caveman:<name>`.
...
- `team ralph` links team pipeline with Ralph-style sequential verification.
- Prefer team mode when independent parallel lanes justify coordination overhead.

## Commit Protocol

Git trailers preserve decision context in every commit message.

### Format
- Intent line first: why change was made
- Optional body with context and rationale
- Structured trailers when applicable

### Common trailers
- `Constraint:` active constraint shaping decision
