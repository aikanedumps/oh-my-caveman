...
Research = multi-stage workflow that decomposes complex research goals into parallel investigations:
...
```
/oh-my-caveman:sciomc <goal>                    # Standard research with user checkpoints
/oh-my-caveman:sciomc AUTO: <goal>              # Fully autonomous until complete
/oh-my-caveman:sciomc status                    # Check current research session status
/oh-my-caveman:sciomc resume                    # Resume interrupted research session
/oh-my-caveman:sciomc list                      # List all research sessions
/oh-my-caveman:sciomc report <session-id>       # Generate report for session
```

### Quick Examples

```
/oh-my-caveman:sciomc What are the performance characteristics of different sorting algorithms?
/oh-my-caveman:sciomc AUTO: Analyze authentication patterns in this codebase
/oh-my-caveman:sciomc How does the error handling work across the API layer?
```
...
Given research goal, decompose into 3-7 independent stages:
...
Task(subagent_type="oh-my-caveman:scientist", model="haiku", prompt="[RESEARCH_STAGE:1] Investigate...")

// Stage 2 - Standard analysis
Task(subagent_type="oh-my-caveman:scientist", model="sonnet", prompt="[RESEARCH_STAGE:2] Analyze...")

// Stage 3 - Complex reasoning
Task(subagent_type="oh-my-caveman:scientist", model="opus", prompt="[RESEARCH_STAGE:3] Deep analysis of...")
```
...
Task(subagent_type="oh-my-caveman:scientist", model="sonnet", prompt="
...
AUTO mode runs complete research workflow autonomously with loop control.
...
4. **Cancellation:** `/oh-my-caveman:cancel` or "stop", "cancel"

### AUTO Mode Example

```
/oh-my-caveman:sciomc AUTO: Comprehensive security analysis of the authentication system
...
Task(subagent_type="oh-my-caveman:scientist", model="haiku", prompt="[STAGE:1] Analyze src/api/...")
Task(subagent_type="oh-my-caveman:scientist", model="haiku", prompt="[STAGE:2] Analyze src/utils/...")
Task(subagent_type="oh-my-caveman:scientist", model="haiku", prompt="[STAGE:3] Analyze src/components/...")
```
...
Task(subagent_type="oh-my-caveman:scientist", model="sonnet", prompt="[HYPOTHESIS:A] Test if caching improves...")
Task(subagent_type="oh-my-caveman:scientist", model="sonnet", prompt="[HYPOTHESIS:B] Test if batching reduces...")
Task(subagent_type="oh-my-caveman:scientist", model="sonnet", prompt="[HYPOTHESIS:C] Test if lazy loading helps...")
```
...
Task(subagent_type="oh-my-caveman:scientist", model="opus", prompt="
[CROSS_VALIDATION]
...
| Command | Action |
|---------|--------|
| `/oh-my-caveman:sciomc status` | Show current session progress |
| `/oh-my-caveman:sciomc resume` | Resume most recent interrupted session |
| `/oh-my-caveman:sciomc resume <session-id>` | Resume specific session |
| `/oh-my-caveman:sciomc list` | List all sessions with status |
| `/oh-my-caveman:sciomc report <session-id>` | Generate/regenerate report |
| `/oh-my-caveman:sciomc cancel` | Cancel current session (preserves state) |

## Tag Extraction

Scientists use structured tags for findings. Extract with these patterns:
...
Progress preserved in `.omc/research/{session-id}/` for resume.
