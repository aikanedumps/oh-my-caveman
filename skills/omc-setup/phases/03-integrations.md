...
```bash
grep -q "oh-my-caveman" "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/settings.json" && echo "Plugin verified" || echo "Plugin NOT found - run: claude /install-plugin oh-my-caveman"
```
...
Invoke the AskUserQuestion tool with this exact shape (all fields required — `question`, `header`, `multiSelect`, and at least 2 `options`):

```
{
  "questions": [{
    "question": "Configure MCP servers for enhanced capabilities (Context7, Exa search, GitHub, etc.)?",
    "header": "MCP servers",
    "multiSelect": false,
    "options": [
      {"label": "Yes, configure MCP", "description": "Run the mcp-setup skill to add web search, GitHub, and other MCP integrations."},
      {"label": "No, skip", "description": "Continue without MCP servers; can configure later via /oh-my-caveman:mcp-setup."}
    ]
  }]
}
```

If user picks **Yes**, invoke the mcp-setup skill:
```
/oh-my-caveman:mcp-setup
```
...
Or by running `/oh-my-caveman:omc-setup --force` and choosing to enable teams.
