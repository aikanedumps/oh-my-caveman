...
Configure Model Context Protocol (MCP) servers to extend Claude Code capabilities with external tools: web search, file system access, GitHub integration.

## Overview

MCP servers provide additional tools Claude Code agents can use. Skill configures popular MCP servers via `claude mcp add` CLI.

## Step 1: Show Available MCP Servers

Present user with available MCP server options using AskUserQuestion:
...
6. **Custom** - Add custom MCP server
...
Use `claude mcp add` command to configure each MCP server. CLI handles settings.json updates and merging automatically.
...
> Note: Docker option requires Docker installed. HTTP option simpler but may have different capabilities.

## Step 4: Verify Installation

After configuration, verify MCP servers properly set up:
...
- Run /oh-my-caveman:omc-doctor to diagnose issues

MANAGING MCP SERVERS:
- Add more servers: /oh-my-caveman:mcp-setup or `claude mcp add ...`
...
Then construct and run appropriate `claude mcp add` command:
...
- Built-in websearch deprioritized when exa configured
- Run `claude mcp list` to confirm servers active

### Removing or Updating Server
- Remove: `claude mcp remove <server-name>`
- Update: Remove old server, then add again with new configuration
