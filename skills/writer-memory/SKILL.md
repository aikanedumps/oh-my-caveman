...
Persistent memory system for creative writers, with first-class support for Korean storytelling workflows.

## Overview

Writer Memory maintains context across Claude sessions for fiction writers. Tracks:
...
| Command | Action |
|---------|--------|
| `/oh-my-caveman:writer-memory init <project-name>` | Initialize new project memory |
| `/oh-my-caveman:writer-memory status` | Show memory overview (character count, scene count, etc) |
| `/oh-my-caveman:writer-memory char add <name>` | Add new character |
| `/oh-my-caveman:writer-memory char <name>` | View character details |
| `/oh-my-caveman:writer-memory char update <name> <field> <value>` | Update character field |
| `/oh-my-caveman:writer-memory char list` | List all characters |
| `/oh-my-caveman:writer-memory rel add <char1> <char2> <type>` | Add relationship |
| `/oh-my-caveman:writer-memory rel <char1> <char2>` | View relationship |
| `/oh-my-caveman:writer-memory rel update <char1> <char2> <event>` | Add relationship event |
| `/oh-my-caveman:writer-memory scene add <title>` | Add new scene |
| `/oh-my-caveman:writer-memory scene <id>` | View scene details |
| `/oh-my-caveman:writer-memory scene list` | List all scenes |
| `/oh-my-caveman:writer-memory theme add <name>` | Add theme |
| `/oh-my-caveman:writer-memory world set <field> <value>` | Set world attribute |
| `/oh-my-caveman:writer-memory query <question>` | Query memory naturally (Korean supported) |
| `/oh-my-caveman:writer-memory validate <character> <dialogue>` | Check if dialogue matches character tone |
| `/oh-my-caveman:writer-memory synopsis` | Generate emotion-focused synopsis |
| `/oh-my-caveman:writer-memory export` | Export full memory as readable markdown |
| `/oh-my-caveman:writer-memory backup` | Create manual backup |

## Memory Types

### 캐릭터 메모리 (Character Memory)

Tracks individual character attributes for consistent portrayal:
...
Establishes universe your story inhabits:

| Field | Korean | Description |
|-------|--------|-------------|
| `setting` | 배경 | Time, place, social context |
| `rules` | 규칙 | How world operates (magic systems, social norms) |
...
Captures dynamic between characters over time:
...
| `power_dynamic` | Who has upper hand, if any |
...
| `purpose` | 목적 | Why this scene exists in story |
| `before_after` | 전후 변화 | What changes for characters |

### 테마 메모리 (Theme Memory)

Captures deeper meaning woven through your story:
...
5. **엔딩 정서 잔상** (Ending Emotional Aftertaste)
   - Lingering feeling after story ends
   - Example: "씁쓸한 안도, 불완전한 해피엔딩의 여운"

## Character Validation (캐릭터 검증)

`/validate` command checks if dialogue matches character's established voice.
...
| **Tone Match** | Does emotional register fit? |
...
- **PASS**: Dialogue consistent with character
...
System synthesizes answers from all relevant memory types.

## Behavior

1. **On Init**: Creates `.writer-memory/memory.json` with project metadata and empty collections
2. **Auto-Backup**: Changes backed up before modification to `.writer-memory/backups/`
3. **Korean-First**: Emotion vocabulary uses Korean terms throughout
4. **Session Loading**: Memory loaded on session start for immediate context
...
- Scene ideas captured as notes
- Character insights from analysis sessions preserved
- Cross-reference between notepad and memory

### With Architect Agent
For complex character analysis:
```
Task(subagent_type="oh-my-caveman:architect",
     model="opus",
...
4. **Query Before Difficult Scenes**: Let system remind you of context
...
- Intentional breaks from pattern valid for dramatic moments
