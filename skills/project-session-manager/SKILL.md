...
`psm` is compatibility alias for this canonical skill entrypoint.

> **Quick Start (worktree-first):** Start with `omc teleport` for isolated issue/PR/feature worktree before adding tmux/session orchestration:
> ```bash
...
Automate isolated dev environments using git worktrees and tmux sessions with Claude Code. Enables parallel work across multiple tasks, projects, and repositories.

Canonical slash command: `/oh-my-caveman:project-session-manager` (alias: `/oh-my-caveman:psm`).
...
```json
{
  "aliases": {
    "omc": {
      "repo": "aikanedumps/oh-my-caveman",
      "local": "~/Workspace/oh-my-caveman",
      "default_base": "main"
    }
  },
  "defaults": {
...
To use Jira, add alias with `jira_project` and `provider: "jira"`:
...
**Important:** `repo` field still required for cloning git repository. Jira tracks issues, but work happens in git repo.
...
PSM recognizes `PROJ-123` format as Jira only when `PROJ` is explicitly configured as `jira_project` in aliases. Prevents false positives from branch names like `FIX-123`.
...
# Note: 'psm review' not supported for Jira (no PR concept)
...
Jira CLI handles authentication separately from PSM.
...
When user invokes PSM command, follow this protocol:
...
8. **Launch Claude Code** (unless --no-claude):
   ```bash
   # --dangerously-skip-permissions prevents "Do you trust this directory?" prompt
   # and repeated tool-approval prompts from stalling session (issue #2508).
   tmux send-keys -t "psm:$project_alias:pr-$pr_number" "claude --dangerously-skip-permissions" Enter

   # After claude boots (PSM_CLAUDE_STARTUP_DELAY, default 5s), deliver task.
   # Use -l (literal) so special characters not misinterpreted by tmux.
...
6. **Update registry, create tmux, launch claude**:
   Same as review, but pass issue context as initial task prompt:
   ```bash
   tmux send-keys -t "psm:$project_alias:issue-$issue_number" "claude --dangerously-skip-permissions" Enter
   # After claude boots, deliver task (see PSM_CLAUDE_STARTUP_DELAY):
...
`omc teleport` provides lightweight alternative to full PSM sessions. Creates git worktrees without tmux session management — ideal for quick, isolated development.
