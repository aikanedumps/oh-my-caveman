 ...
 *
 * See: https://github.com/aikanedumps/oh-my-caveman/issues/1007
 */
function resolveTarget(targetPath) {
  ...
  // CLAUDE_PLUGIN_ROOT is e.g. ~/.claude/plugins/cache/omc/oh-my-caveman/4.2.14
  // We look one level up for sibling version directories.
  try {
    const pluginRoot = process.env.CLAUDE_PLUGIN_ROOT;
    if (!pluginRoot) return null;

    const cacheBase = dirname(pluginRoot);          // .../oh-my-caveman/
