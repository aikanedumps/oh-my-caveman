  ...
  try {
    if (existsSync(settingsFile)) {
      ...
      if (statusLineCommand?.includes('omc-hud')) {
        const pluginCacheBase = join(configDir, 'plugins', 'cache', 'oh-my-caveman', 'oh-my-caveman');
  ...
  try {
    ...
    try {
      ...
      if (pluginVersion) {
        ...
        if (updateInfo) {
          messages.push(`<session-restore>\n\n[OMC UPDATE AVAILABLE]\n\nA new version of oh-my-caveman is available: v${updateInfo.latestVersion} (current: v${updateInfo.currentVersion})\n\nTo update, run: omc update\n(This syncs plugin, npm package, and CLAUDE.md together)\n\n</session-restore>\n\n---\n`);
        }
      }
    } catch {}
...
${cleanContent}
    ...
    try {
      const cacheBase = join(configDir, 'plugins', 'cache', 'oh-my-caveman', 'oh-my-caveman');
