...
function isExplicitRalplanSlashInvocation(prompt) {
  return /^\s*\/(?:oh-my-caveman:)?ralplan(?:\s|$)/i.test(prompt);
}

function isExplicitAskSlashInvocation(prompt) {
  return /^\s*\/(?:oh-my-caveman:)?ask\s+(?:claude|codex|gemini)\b/i.test(prompt);
}
...
const ECHO_CONTINUATION = '(?:\\r?\\n[ \\t]*(?:Task:\\s|When FULLY complete \\(after Architect verification\\)|run\\s+\\/oh-my-caveman:cancel).*)*';
...
const SYSTEM_ECHO_SIGNATURES = [
  ...
  /\brun\s+\/oh-my-caveman:cancel\b/i,
...
Arguments: ${args}` : '';
  ...
  const pathStatus = existsSync(skillPath)
    ...
    : `Read fallback: locate skills/${skillName}/SKILL.md in the active oh-my-caveman plugin/install and follow it.`;

  return `[MAGIC KEYWORD: ${skillName.toUpperCase()}]

Skill routing detected: ${skillName}
Preferred invocation: /oh-my-caveman:${skillName}${args ? ` ${args}` : ''}
...
function createMultiSkillInvocation(skills, originalPrompt) {
  ...
  const skillBlocks = skills.map((s, i) => {
    ...
    const pathStatus = existsSync(skillPath)
      ...
      : `Read fallback: locate skills/${s.name}/SKILL.md in the active oh-my-caveman plugin/install`;
    return `### Skill ${i + 1}: ${s.name.toUpperCase()}
Preferred invocation: /oh-my-caveman:${s.name}${argsText}
${pathStatus}`;
  }).join('\n\n');
