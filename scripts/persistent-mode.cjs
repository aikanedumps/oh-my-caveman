 ...
 *
 * See: https://github.com/aikanedumps/oh-my-caveman/issues/213
 */
  ...
  try {
    ...
    if (isAuthoritativeModeActive(stateDir, "ralph", ralph, sessionId) && !isAwaitingConfirmation(ralph.state) && !isStaleState(ralph.state) && isSessionMatch(ralph.state, sessionId)) {
      ...
      if (iteration < maxIter) {
        ...
        const ralphReason = `[RALPH LOOP - ITERATION ${iteration + 1}/${maxIter}] Work is NOT done. Continue working.\nWhen FULLY complete (after Architect verification), run /oh-my-caveman:cancel to cleanly exit ralph mode and clean up all state files. If cancel fails, retry with /oh-my-caveman:cancel --force.\n${ralph.state.prompt ? `Task: ${ralph.state.prompt}` : ""}`;
        console.log(
          JSON.stringify({
            decision: "block",
            reason: ralphReason,
        ...
        writeJsonFile(ralph.path, ralph.state);
        const extendReason = `[RALPH LOOP - EXTENDED] Max iterations reached; extending to ${ralph.state.max_iterations} and continuing. When FULLY complete (after Architect verification), run /oh-my-caveman:cancel (or --force).`;
      ...
      if (phase !== "complete") {
        ...
        if (newCount <= 20) {
          ...
          const cancelGuidance = typeof autopilot.state.session_id === "string" && autopilot.state.session_id === sessionId
            ? " When all phases are complete, run /oh-my-caveman:cancel to cleanly exit and clean up this session's autopilot state files. If cancel fails, retry with /oh-my-caveman:cancel --force."
            : "";
          console.log(
            JSON.stringify({
              decision: "block",
    ...
    if (ralplan.state?.active && !isAwaitingConfirmation(ralplan.state) && !isStaleState(ralplan.state) && isSessionMatch(ralplan.state, sessionId)) {
      ...
      if (!ralplanTerminal && !cancelInProgress) {
        ...
        if (breakerCount > RALPLAN_STOP_BLOCKER_MAX) {
          ...
          const ralplanReason = `[RALPLAN - CONSENSUS PLANNING | REINFORCEMENT ${breakerCount}/${RALPLAN_STOP_BLOCKER_MAX}] The ralplan consensus workflow is active. Continue the Planner/Architect/Critic loop. Do not stop until consensus is reached or the workflow completes. When done, run /oh-my-caveman:cancel to cleanly exit.`;
          console.log(JSON.stringify({
            decision: "block",
      ...
      if (incomplete > 0) {
        ...
        if (newCount <= 20) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[ULTRAPILOT] ${incomplete} workers still running. Continue working. When all workers complete, run /oh-my-caveman:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-caveman:cancel --force.`,
            }),
          );
          return;
        }
      ...
      if (pending > 0) {
        ...
        if (newCount <= 15) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[SWARM ACTIVE] ${pending} tasks remain. Continue working. When all tasks are done, run /oh-my-caveman:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-caveman:cancel --force.`,
            }),
          );
          return;
        }
      ...
      if (currentStage < totalStages) {
        ...
        if (newCount <= 15) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[PIPELINE - Stage ${currentStage + 1}/${totalStages}] Pipeline not complete. Continue working. When all stages complete, run /oh-my-caveman:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-caveman:cancel --force.`,
            }),
          );
          return;
        }
      ...
      if (phase) {
        ...
        if (newCount <= 20) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[TEAM - Phase: ${phase}] Team mode active. Continue working. When all team tasks complete, run /oh-my-caveman:cancel to cleanly exit. If cancel fails, retry with /oh-my-caveman:cancel --force.`,
            }),
          );
          return;
        }
      ...
      if (phase) {
        ...
        if (newCount <= 20) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[OMC TEAMS - Phase: ${phase}] OMC Teams workers active. Continue working. When all workers complete, run /oh-my-caveman:cancel to cleanly exit. If cancel fails, retry with /oh-my-caveman:cancel --force.`,
            }),
          );
          return;
        }
    ...
    if (ultraqa.state?.active && !isStaleState(ultraqa.state) && isSessionMatch(ultraqa.state, sessionId)) {
      ...
      if (cycle < maxCycles && !ultraqa.state.all_passing) {
        ...
        console.log(
          JSON.stringify({
            decision: "block",
            reason: `[ULTRAQA - Cycle ${cycle + 1}/${maxCycles}] Tests not all passing. Continue fixing. When all tests pass, run /oh-my-caveman:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-caveman:cancel --force.`,
          }),
        );
        return;
      }
    ...
    if (
      ...
      if (totalIncomplete > 0) {
        ...
        reason += ` ${totalIncomplete} incomplete ${itemType} remain. Continue working. When all work is complete, run /oh-my-caveman:cancel to cleanly exit ultrawork mode and clean up state files.`;
      } else if (newCount >= 5) {
        // Strong directive: LLM must call cancel NOW
        reason += ` No incomplete tasks detected. You MUST invoke /oh-my-caveman:cancel immediately to exit ultrawork mode and clean up state files. Call state_clear(mode="ultrawork") if the cancel skill is unavailable.`;
      } else if (newCount >= 3) {
        // Reinforce clean-exit guidance once no tracked work remains.
        reason += ` If all work is complete, run /oh-my-caveman:cancel to cleanly exit ultrawork mode and clean up state files. If cancel fails, retry with /oh-my-caveman:cancel --force. Otherwise, continue working.`;
      } else {
        // Early iterations with no tasks yet still need an immediately visible exit path.
        reason += ` No incomplete tasks detected. If all work is complete, run /oh-my-caveman:cancel to cleanly exit ultrawork mode and clean up state files. Otherwise, continue working - create Tasks to track your progress.`;
      }
