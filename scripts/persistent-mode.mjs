  ...
  try {
      ...
      if (sessionMatches) {
        ...
        if (hardMax > 0 && maxIter >= hardMax) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[RALPH LOOP - HARD LIMIT] Reached hard max iterations (${hardMax}). Mode auto-disabled. Restart with /oh-my-caveman:ralph if needed.`,
            }),
          );
          return;
        }
        ...
        const ralphExtendedReason = `[RALPH LOOP - EXTENDED] Max iterations reached; extending to ${ralph.state.max_iterations} and continuing. When FULLY complete (after Architect verification), run /oh-my-caveman:cancel (or --force).`;
        console.log(
          JSON.stringify({
            decision: "block",
    ...
    if (
      ...
      if (incomplete > 0) {
        ...
        if (newCount <= 20) {
          ...
          let reason = `[ULTRAPILOT] ${incomplete} workers still running. Continue working. When all workers complete, run /oh-my-caveman:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-caveman:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (pending > 0) {
        ...
        if (newCount <= 15) {
          ...
          let reason = `[SWARM ACTIVE] ${pending} tasks remain. Continue working. When all tasks are done, run /oh-my-caveman:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-caveman:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (currentStage < totalStages) {
        ...
        if (newCount <= 15) {
          ...
          let reason = `[PIPELINE - Stage ${currentStage + 1}/${totalStages}] Pipeline not complete. Continue working. When all stages complete, run /oh-my-caveman:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-caveman:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
      ...
      if (sessionMatches) {
        ...
        if (phase) {
          ...
          if (newCount <= 20) {
            ...
            let reason = `[TEAM - Phase: ${phase}] Team mode active. Continue working. When all team tasks complete, run /oh-my-caveman:cancel to cleanly exit. If cancel fails, retry with /oh-my-caveman:cancel --force.`;
            if (errorGuidance) {
              reason = errorGuidance + reason;
            }
      ...
      if (sessionMatches) {
        ...
        if (phase) {
          ...
          if (newCount <= 20) {
            ...
            let reason = `[OMC TEAMS - Phase: ${phase}] OMC Teams workers active. Continue working. When all workers complete, run /oh-my-caveman:cancel to cleanly exit. If cancel fails, retry with /oh-my-caveman:cancel --force.`;
            if (errorGuidance) {
              reason = errorGuidance + reason;
            }
    ...
    if (
      ...
      if (cycle < maxCycles && !ultraqa.state.all_passing) {
        ...
        let reason = `[ULTRAQA - Cycle ${cycle + 1}/${maxCycles}] Tests not all passing. Continue fixing. When all tests pass, run /oh-my-caveman:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-caveman:cancel --force.`;
        if (errorGuidance) {
          reason = errorGuidance + reason;
        }
