...
function checkRalphRubyDependency() {
  try {
    ...
    console.log('[OMC] After installing Ruby, restart Claude Code and rerun /oh-my-caveman:omc-setup if needed.');
  }
}
...
try {
  ...
  // Skip patching when running inside the source repo: would bake the developer's
  // local node path into a tracked file and contaminate commits (issue surfaced
  // post-v2.0.0 — committed binary path broke installs on other machines).
  const inSourceRepo = existsSync(join(__dirname, '..', '.git'));
  if (existsSync(hooksJsonPath) && !inSourceRepo) {
