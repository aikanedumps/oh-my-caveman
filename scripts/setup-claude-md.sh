...
DOWNLOAD_URL="https://raw.githubusercontent.com/aikanedumps/oh-my-caveman/main/docs/CLAUDE.md"
...
resolve_active_plugin_root() {
  ...
  if [ -f "$installed_plugins" ] && command -v jq >/dev/null 2>&1; then
    ...
    active_path=$(jq -r '
      (.plugins // .)
      | to_entries[]
      | select(.key | startswith("oh-my-caveman"))
...
if [ -f "$CONFIG_DIR/settings.json" ] && grep -q "oh-my-caveman" "$CONFIG_DIR/settings.json"; then
  echo "Plugin verified"
else
  echo "Plugin NOT found - run: claude /install-plugin oh-my-caveman"
fi
