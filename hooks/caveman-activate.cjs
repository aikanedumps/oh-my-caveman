...
const os = require('os');
const { getDefaultMode } = require('./caveman-config.cjs');
...
// 2. Emit caveman ruleset for the active intensity level.
//    Reads SKILL.md at runtime — single source of truth, no hardcoded duplication.
...
try {
  ...
  try {
    hasStatusline = !!JSON.parse(fs.readFileSync(settingsPath, 'utf8')).statusLine;
  } catch (e) {}
