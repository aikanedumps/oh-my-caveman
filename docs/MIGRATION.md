...
This guide covers all migration paths for oh-my-caveman. Find your current version below.
...
| -------------------- | -------------------------------------- |
| `cancel-autopilot`   | `/oh-my-caveman:cancel`             |
| `cancel-ralph`       | `/oh-my-caveman:cancel`             |
| `cancel-ultrawork`   | `/oh-my-caveman:cancel`             |
| `cancel-ultraqa`     | `/oh-my-caveman:cancel`             |
| `omc-default`        | `/oh-my-caveman:omc-setup --local`  |
| `omc-default-global` | `/oh-my-caveman:omc-setup --global` |
| `planner`            | `/oh-my-caveman:plan`               |
...
```bash
/oh-my-caveman:cancel-ralph      # Cancel ralph specifically
/oh-my-caveman:omc-default       # Configure local project
/oh-my-caveman:planner "task"    # Start planning
```

**After v3.5.3:**

```bash
/oh-my-caveman:cancel            # Auto-detects and cancels any active mode
/oh-my-caveman:omc-setup --local # Configure local project
/oh-my-caveman:plan "task"       # Start planning (includes interview mode)
```
...
```bash
/oh-my-caveman:plan --consensus "task"  # Iterative planning with Critic review
/oh-my-caveman:ralplan "task"           # Alias for plan --consensus
```
...
**Before 3.0:** Explicitly invoke 25+ commands like `/oh-my-caveman:ralph "task"`, `/oh-my-caveman:ultrawork "task"`
...
- **Project/brand name**: `oh-my-caveman` (GitHub repo, plugin name, commands)
- **npm package name**: `oh-my-caveman` (unchanged)

> **Why the difference?** The npm package name `oh-my-caveman` was kept for backward compatibility with existing installations. The project, GitHub repository, plugin, and all commands use `oh-my-caveman`.

#### NPM Install Command (unchanged)

```bash
npm i -g oh-my-caveman@latest
```
...
/oh-my-caveman:ralph "implement user authentication"       # Persistence mode
/oh-my-caveman:ultrawork "refactor the API layer"          # Maximum parallelism
/oh-my-caveman:planner "plan the new dashboard"            # Planning interview
/oh-my-caveman:deepsearch "find database schema files"     # Deep search
/oh-my-caveman:git-master "commit these changes"           # Git expertise
/oh-my-caveman:deepinit ./src                              # Index codebase
/oh-my-caveman:analyze "why is this test failing?"         # Deep analysis
```
...
| -------------------------------------- | -------------------------------------------------- | ---------------------- |
| `/oh-my-caveman:ralph "task"`       | Say "don't stop until done" OR use `ralph` keyword | ✅ YES (both ways)     |
| `/oh-my-caveman:ultrawork "task"`   | Say "fast" or "parallel" OR use `ulw` keyword      | ✅ YES (both ways)     |
| `/oh-my-caveman:ultrawork-ralph`    | Say "ralph ulw:" prefix                            | ✅ YES (keyword combo) |
| `/oh-my-caveman:planner "task"`     | Say "plan this" OR use `plan` keyword              | ✅ YES (both ways)     |
| `/oh-my-caveman:plan "description"` | Start planning naturally                           | ✅ YES                 |
| `/oh-my-caveman:review [path]`      | Invoke normally                                    | ✅ YES (unchanged)     |
| `/oh-my-caveman:deepsearch "query"` | Say "find" or "search"                             | ✅ YES (auto-detect)   |
| `/oh-my-caveman:analyze "target"`   | Say "analyze" — routes to debugger/architect agent | ✅ YES (keyword route) |
| `/oh-my-caveman:deepinit [path]`    | Invoke normally                                    | ✅ YES (unchanged)     |
| `/oh-my-caveman:git-master`         | Say "git", "commit", "atomic commit"               | ✅ YES (auto-detect)   |
| `/oh-my-caveman:frontend-ui-ux`     | Say "UI", "styling", "component", "design"         | ✅ YES (auto-detect)   |
| `/oh-my-caveman:note "content"`     | Say "remember this" or "save this"                 | ✅ YES (auto-detect)   |
| `/oh-my-caveman:cancel-ralph`       | Say "stop", "cancel", or "abort"                   | ✅ YES (auto-detect)   |
| `/oh-my-caveman:omc-doctor`         | Invoke normally                                    | ✅ YES (unchanged)     |
...
No more `/oh-my-caveman:cancel-ralph` - just say "cancel"!
...
- Package name: `oh-my-caveman` → `oh-my-caveman`
...
3. **Test a simple command**:
   Run `/oh-my-caveman:omc-help` in Claude Code to ensure the plugin is loaded correctly.
...
```
Before: "OK, I need to use /oh-my-caveman:ultrawork for speed..."
...
#### 3. Learned Skills (`/oh-my-caveman:skillify`)

Extract reusable insights from problem-solving. `/oh-my-caveman:learner` remains as a deprecated compatibility alias:
...
Run `/oh-my-caveman:hud setup` to install. Presets: minimal, focused, full.
...
Or use `/oh-my-caveman:note` to save discoveries manually:

```bash
/oh-my-caveman:note Project uses PostgreSQL with Prisma ORM
```
...
```bash
/oh-my-caveman:ralph-init "implement OAuth with multiple providers"
    ↓
...
```bash
/oh-my-caveman:pipeline explore:haiku -> architect:opus -> executor:sonnet
```
...
- `/oh-my-caveman:cancel-ralph` (deprecated)
- `/oh-my-caveman:cancel-ultraqa` (deprecated)
- `/oh-my-caveman:cancel-ultrawork` (deprecated)
- `/oh-my-caveman:cancel-autopilot` (deprecated)

Use `/oh-my-caveman:cancel` instead.
...
```typescript
Task(
  (subagent_type = "oh-my-caveman:explore-high"),
  (model = "opus"),
...
Users set their default mode preference via `/oh-my-caveman:omc-setup`.
...
- Watch the [GitHub repository](https://github.com/aikanedumps/oh-my-caveman) for announcements
...
```
/oh-my-caveman:ultrawork "implement the todo list feature"
```
...
```
/oh-my-caveman:ralph "debug the memory leak"
```
...
```
/oh-my-caveman:planner "design the new authentication system"
```
...
Apply oh-my-caveman to current project only:

```
/oh-my-caveman:omc-default
```
...
```
/oh-my-caveman:omc-default-global
```
...
A: Keep using them! `/oh-my-caveman:ralph`, `/oh-my-caveman:ultrawork`, and `/oh-my-caveman:plan` work. Note: `/oh-my-caveman:planner` now redirects to `/oh-my-caveman:plan`.

**Q: How do I know what Claude is doing?**
A: Claude announces major behaviors: "I'm activating ralph-loop..." or set up `/oh-my-caveman:hud` for real-time status.
...
- **Diagnose issues**: Run `/oh-my-caveman:omc-doctor`
- **See all commands**: Run `/oh-my-caveman:omc-help`
- **View real-time status**: Run `/oh-my-caveman:hud setup`
- **Review detailed changelog**: See [CHANGELOG.md](../CHANGELOG.md)
- **Report bugs**: [GitHub Issues](https://github.com/aikanedumps/oh-my-caveman/issues)
