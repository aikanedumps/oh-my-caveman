...
|---|---|---|
| **Claude Code plugin** (`oh-my-caveman@omc`) | In-session skills, agents, hooks, statusline, MCP servers — the `/autopilot`, `/ralph`, `/ultrawork`, `/team` slash commands | Marketplace plugin install (Step 1–2 below) |
| **Terminal CLI** (`omc` binary, package `oh-my-caveman`) | Shell commands: `omc setup`, `omc update`, `omc team`, `omc ask`, and a hard-deprecated `omc autoresearch` shim | `npm i -g oh-my-caveman@latest` |
...
> Older versions of this doc said OMC was "plugin-only". That was incorrect: the `omc` CLI is the canonical entry point for `omc setup`/`omc update` and is published on npm as `oh-my-caveman`. See the [Quick Start in README.md](../README.md#quick-start) for the same two-path layout.
...
> yet. The warning is tracked in [#2913](https://github.com/aikanedumps/oh-my-caveman/issues/2913)
...
> ⚠️ **Warning:** After a plugin update, run `/oh-my-caveman:omc-setup` again to apply the latest configuration.
...
OMC's default behavior is also configured via `CLAUDE.md` files. Running `/oh-my-caveman:omc-setup` generates this file automatically.
