...
Complete reference for oh-my-caveman. For quick start, see the main [README.md](../README.md).
...
- **New projects**: Run `/oh-my-caveman:omc-setup --local` in each project that needs omc

> **NOTE**: After updating the plugin (via `npm update`, `git pull`, or Claude Code's plugin update), you MUST re-run `/oh-my-caveman:omc-setup` to apply the latest CLAUDE.md changes.
...
```bash
omc --plugin-dir /path/to/oh-my-caveman setup --plugin-dir-mode
```
...
```bash
export OMC_PLUGIN_ROOT=/path/to/oh-my-caveman
claude --plugin-dir /path/to/oh-my-caveman
```
...
```bash
omc doctor --plugin-dir /path/to/oh-my-caveman
omc doctor conflicts --plugin-dir /path/to/oh-my-caveman
```
...
- Skill entrypoint: `/oh-my-caveman:ask <claude|codex|gemini> <prompt>` routes to this command
...
Always use `oh-my-caveman:` prefix when calling via Task tool.
...
| ------------------------- | ---------------------------------------------------------------- | ------------------------------------------- |
| `ai-slop-cleaner`         | Anti-slop cleanup workflow with optional reviewer-only `--review` pass | `/oh-my-caveman:ai-slop-cleaner`         |
| `ask`                     | Ask Claude, Codex, or Gemini via local CLI and capture a reusable artifact | `/oh-my-caveman:ask`               |
| `autoresearch`            | Stateful single-mission evaluator-driven improvement loop           | `/oh-my-caveman:autoresearch`            |
| `autopilot`               | Full autonomous execution from idea to working code              | `/oh-my-caveman:autopilot`               |
| `cancel`                  | Unified cancellation for active modes                            | `/oh-my-caveman:cancel`                  |
| `ccg`                     | Tri-model workflow via `ask codex` + `ask gemini`, then Claude synthesis | `/oh-my-caveman:ccg`                     |
| `configure-notifications` | Configure notification integrations (Telegram, Discord, Slack) via natural language | `/oh-my-caveman:configure-notifications` |
| `deep-dive`               | Two-stage trace → deep-interview pipeline with context handoff   | `/oh-my-caveman:deep-dive`               |
| `deep-interview`          | Socratic deep interview with ambiguity gating                    | `/oh-my-caveman:deep-interview`          |
| `deepinit`                | Generate hierarchical AGENTS.md docs                             | `/oh-my-caveman:deepinit`                |
| `external-context`        | Parallel document-specialist research                            | `/oh-my-caveman:external-context`        |
| `hud`                     | Configure HUD/statusline                                         | `/oh-my-caveman:hud`                     |
| `skillify`                | Extract reusable skill from session                              | `/oh-my-caveman:skillify`                |
| `learner`                 | **Deprecated** compatibility alias for `skillify`                | `/oh-my-caveman:learner`                 |
| `mcp-setup`               | Configure MCP servers                                            | `/oh-my-caveman:mcp-setup`               |
| `omc-doctor`              | Diagnose and fix installation issues                             | `/oh-my-caveman:omc-doctor`              |
| `omc-plan`                | Planning workflow (`/plan` safe alias; bundled directory ID is `plan`) | `/oh-my-caveman:plan`                    |
| `omc-reference`           | Detailed OMC agent/tools/team/commit reference skill             | Auto-loaded reference only                  |
| `omc-setup`               | One-time setup wizard                                            | `/oh-my-caveman:omc-setup`               |
| `omc-teams`               | Spawn `claude`/`codex`/`gemini` tmux workers for parallel execution | `/oh-my-caveman:omc-teams`             |
| `project-session-manager` | Manage isolated dev environments (git worktrees + tmux)          | `/oh-my-caveman:project-session-manager` |
| `psm` | **Deprecated** compatibility alias for `project-session-manager` | `/oh-my-caveman:psm` |
| `ralph`                   | Persistence loop until verified completion                       | `/oh-my-caveman:ralph`                   |
| `ralplan`                 | Consensus planning alias for `/plan --consensus`                 | `/oh-my-caveman:ralplan`                 |
| `release`                 | Automated release workflow                                       | `/oh-my-caveman:release`                 |
| `self-improve`            | Autonomous evolutionary code improvement engine with tournament selection; artifacts are topic-scoped under `.omc/self-improve/topics/<topic-slug>/` by default, with flat `.omc/self-improve/` preserved for legacy single-track resumes | `/oh-my-caveman:self-improve`    |
| `setup`                   | Unified setup entrypoint for install, diagnostics, and MCP configuration | `/oh-my-caveman:setup`              |
| `sciomc`                  | Parallel scientist orchestration                                 | `/oh-my-caveman:sciomc`                  |
| `skill`                   | Manage local skills (list/add/remove/search/edit)                | `/oh-my-caveman:skill`                   |
| `team`                    | Coordinated multi-agent workflow                                 | `/oh-my-caveman:team`                    |
| `trace`                   | Evidence-driven tracing lane with parallel tracer hypotheses     | `/oh-my-caveman:trace`                   |
| `ultraqa`                 | QA cycle until goal is met                                       | `/oh-my-caveman:ultraqa`                 |
| `ultrawork`               | Maximum parallel throughput mode                                 | `/oh-my-caveman:ultrawork`               |
| `visual-verdict`          | Structured visual QA verdict for screenshot/reference comparisons | `/oh-my-caveman:visual-verdict`          |
| `wiki`                    | LLM Wiki — persistent markdown knowledge base that compounds across sessions | `/oh-my-caveman:wiki`           |
| `writer-memory`           | Agentic memory system for writing projects                       | `/oh-my-caveman:writer-memory`           |
...
Each installed skill is exposed as `/oh-my-caveman:<skill-name>`. The skills table above is the full runtime-backed list; the commands below highlight common entrypoints and aliases. Compatibility keyword modes like `deep-analyze` and `tdd` are prompt-triggered behaviors, not standalone slash commands.

| Command                                     | Description                                                                                   |
| ------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `/oh-my-caveman:ai-slop-cleaner <target>`    | Run the anti-slop cleanup workflow (`--review` for reviewer-only pass)                    |
| `/oh-my-caveman:ask <claude\|codex\|gemini> <prompt>` | Route a prompt through the selected advisor CLI and capture an ask artifact         |
| `/oh-my-caveman:autopilot <task>`            | Full autonomous execution                                                                  |
| `/oh-my-caveman:configure-notifications`     | Configure notification integrations                                                       |
| `/oh-my-caveman:deep-dive <problem>`         | Run the trace → deep-interview pipeline                                                   |
| `/oh-my-caveman:deep-interview <idea>`       | Socratic interview with ambiguity scoring before execution                                 |
| `/oh-my-caveman:deepinit [path]`             | Index codebase with hierarchical AGENTS.md files                                           |
| `/oh-my-caveman:mcp-setup`                   | Configure MCP servers                                                                      |
| `/oh-my-caveman:omc-doctor`                  | Diagnose and fix installation issues                                                       |
| `/oh-my-caveman:plan <description>`          | Start planning session (supports consensus structured deliberation)                        |
| `/oh-my-caveman:omc-setup`                   | One-time setup wizard                                                                      |
| `/oh-my-caveman:omc-teams <N>:<agent> <task>`       | Spawn `claude`/`codex`/`gemini` tmux workers for legacy parallel execution                |
| `/oh-my-caveman:project-session-manager <arguments>` | Manage isolated dev environments with git worktrees + tmux                         |
| `/oh-my-caveman:psm <arguments>`             | Deprecated alias for project session manager                                               |
| `/oh-my-caveman:ralph <task>`                | Self-referential loop until task completion (`--critic=architect|critic|codex`)           |
| `/oh-my-caveman:ralplan <description>`       | Iterative planning with consensus structured deliberation (`--deliberate` for high-risk mode) |
| `/oh-my-caveman:release`                     | Automated release workflow                                                                 |
| `/oh-my-caveman:setup`                       | Unified setup entrypoint (`setup`, `setup doctor`, `setup mcp`)                           |
| `/oh-my-caveman:sciomc <topic>`              | Parallel research orchestration                                                            |
| `/oh-my-caveman:team <N>:<agent> <task>`     | Coordinated native team workflow                                                           |
| `/oh-my-caveman:trace`                       | Evidence-driven tracing lane that orchestrates parallel tracer hypotheses in team mode     |
| `/oh-my-caveman:ultraqa <goal>`              | Autonomous QA cycling workflow                                                             |
| `/oh-my-caveman:ultrawork <task>`            | Maximum performance mode with parallel agents                                              |
| `/oh-my-caveman:visual-verdict <task>`       | Structured visual QA verdict for screenshot/reference comparisons                          |
...
When present, OMC appends a standardized **Skill Pipeline** section to the rendered skill prompt so the current stage, handoff artifact, and explicit next `Skill("oh-my-caveman:...")` invocation are carried forward consistently.
...
oh-my-caveman includes comprehensive monitoring for agent performance, token usage, and debugging parallel workflows.
...
| Issue                 | Solution                                                                         |
| --------------------- | -------------------------------------------------------------------------------- |
| Commands not found    | Re-run `/oh-my-caveman:omc-setup`                                             |
...
| Token limit errors    | Use `/oh-my-caveman:` for token-efficient execution                           |
...
```
/plugin uninstall oh-my-caveman@oh-my-caveman
```
