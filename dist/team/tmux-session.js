...
import { tmuxExec, tmuxExecAsync, tmuxCmd, tmuxCmdAsync } from '../cli/tmux-utils.js';
...
export function validateTmux(hasTmuxContext = false) {
    ...
    try {
        tmuxCmd(['-V'], { stripTmux: true, timeout: 5000, stdio: 'pipe' });
    }
    catch {
...
export function listActiveSessions(teamName) {
    ...
    try {
        // Use tmuxCmd which auto-routes to shell execution for format strings
        // containing #{} to prevent MSYS2/Git Bash from stripping curly braces.
        const output = tmuxCmd(['list-sessions', '-F', '#{session_name}'], {
