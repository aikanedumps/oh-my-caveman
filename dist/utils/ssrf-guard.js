...
function hostMatchesNoProxy(hostname, noProxy) {
    ...
    for (const raw of noProxy.split(',')) {
        ...
        if (entry.startsWith('.')) {
            ...
            if (lower === entry.slice(1) || lower.endsWith(entry))
                return true;
        }
...
function isProxiedHost(hostname) {
    ...
    if (hostMatchesNoProxy(hostname, noProxy))
        return false;
    return true;
}
/**
...
function isBlockedIPv4(addr) {
    ...
    if (/^(22[4-9]|23[0-9]|24[0-9]|25[0-5])\./.test(addr))
        return true;
    return false;
}
...
function isBlockedIPv6(addr) {
    ...
    if (/^(?:0{0,4}:)*:?0{0,3}1$/i.test(lower) || lower === '::1')
        return true;
    ...
    if (/^fe[89ab][0-9a-f]:/i.test(lower))
        return true;
    // Unspecified ::
    if (lower === '::' || /^0{0,4}(?::0{0,4})*$/.test(lower))
        return true;
    return false;
}
function validateResolvedAddress(addr, family) {
    ...
    if (mapped !== null) {
        if (isBlockedIPv4(mapped))
            return { allowed: false, blockedAddress: addr };
        return { allowed: true };
    }
    if (family === 4) {
...
export function validatedLookup(hostname, options, callback) {
    ...
    dns.lookup(hostname, { all: true, family: wantFamily, hints: optsObj.hints }, (err, addresses) => {
        ...
        if (!addresses || addresses.length === 0) {
            const e = new Error(`[SSRF Guard] DNS lookup for '${hostname}' returned no addresses`);
            e.code = 'ENOTFOUND';
            deliverError(e);
            return;
        }
        ...
        if (!allowPrivate && !proxied) {
            for (const a of addresses) {
                ...
                if (!v.allowed) {
                    const e = new Error(`[SSRF Guard] Hostname '${hostname}' resolved to blocked address '${v.blockedAddress}' (private/loopback/link-local). Refusing connection to prevent DNS rebinding.`);
                    e.code = 'EAI_BLOCKED';
                    deliverError(e);
                    return;
                }
