...
/**
 * Detect whether a statusLine config belongs to oh-my-caveman.
 *
...
export function isOmcHook(command) {
    ...
    const omcPattern = /(?:^|[\/\\_-])omc(?:$|[\/\\_-])/i;
    const fullNamePattern = /oh-my-c(?:aveman|laudecode)/i;
    if (omcPattern.test(command) || fullNamePattern.test(command)) {
        return true;
    }
...
export function getInstalledOmcPluginRoots() {
    ...
    try {
        ...
        for (const [pluginId, entries] of Object.entries(plugins)) {
            if (!pluginId.toLowerCase().includes('oh-my-caveman') || !Array.isArray(entries)) {
                continue;
            }
    ...
    try {
        ...
        for (const [marketplaceId, entry] of Object.entries(raw)) {
            const isOmcMarketplace = marketplaceId.toLowerCase().includes('omc')
                || marketplaceId.toLowerCase().includes('oh-my-caveman');
            if (!isOmcMarketplace) {
                continue;
            }
...
function getGlobalInstalledPackageRoot() {
    try {
        ...
        const globalPackageRoot = join(npmRoot, 'oh-my-caveman');
    ...
    try {
        ...
        for (const candidate of [settings.enabledPlugins, settings.plugins]) {
            if (Array.isArray(candidate)) {
                if (candidate.some(plugin => typeof plugin === 'string' && plugin.toLowerCase().includes('oh-my-caveman'))) {
                    return true;
                }
            }
            else if (candidate && typeof candidate === 'object') {
                if (Object.entries(candidate).some(([pluginId, value]) => pluginId.toLowerCase().includes('oh-my-caveman') && value !== false)) {
                    return true;
                }
            }
        }
...
export function extractOmcVersionFromClaudeMd(content) {
    ...
    const headingMatch = content.match(/^#\s+oh-my-caveman.*?\b(v?\d+\.\d+\.\d+(?:[-+][^\s]+)?)\b/m);
    if (headingMatch?.[1]) {
