 ...
 * `<configDir>/plugins/cache/omc/oh-my-caveman/99.99.99/dist/hud/index.js`.
...
function makeStubConfigDir(rootDir) {
    ...
    const stubDir = join(configDir, 'plugins', 'cache', 'omc', 'oh-my-caveman', '99.99.99', 'dist', 'hud');
 ...
 * `oh-my-caveman` and silently satisfy the npm fallback step.
 */
...
describe('HUD wrapper — OMC_PLUGIN_ROOT resolution', () => {
    ...
    it('case 6: cache step is semver-aware — stable beats prerelease with same [M.m.p]', () => {
        ...
        const cacheBase = join(configDir, 'plugins', 'cache', 'omc', 'oh-my-caveman');
    ...
    it('case 5: symmetry — installer source and plugin-setup.mjs both produce identical wrapper text', async () => {
        ...
        expect(txt).not.toContain('Workspace/oh-my-caveman');
        expect(txt).not.toContain('projects/oh-my-caveman');
    });
