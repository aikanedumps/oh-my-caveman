...
describe('release generation', () => {
    it('extracts a deduped PR set from squash and merge subjects', () => {
        const prNumbers = extractPullRequestNumbers([
            ...
            'Merge pull request #2146 from aikanedumps/issue-2143-omc-launch-followup',
            'Merge pull request #2162 from aikanedumps/release/4.10.2',
            'feat(hud): add configurable call count icon format (#2151)',
        ]);
    ...
    it('derives sorted deduped contributor handles from PR and compare metadata', () => {
        const contributors = deriveContributorLogins([
            ...
            { author: 'EthanJStark' },
            { author: 'blue-int' },
        ], ['tjsingleton', 'DdangJin', 'aikanedumps', 'EthanJStark', null]);
    ...
    it('keeps non-conventional PRs in other changes and renders exact PR counts', () => {
        const pullRequests = [
            ...
            { number: '2137', title: 'Fix team tmux pane geometry collapse and bundled agent path resolution', author: 'aikanedumps', headRefName: 'fix-issue-2135-pane-geometry' },
            { number: '2144', title: 'fix: preserve existing global CLAUDE.md during setup', author: 'aikanedumps', headRefName: 'issue-2143-safe-setup-config' },
            { number: '2146', title: 'fix: follow up #2143 with explicit overwrite choice + omc launch profile', author: 'aikanedumps', headRefName: 'issue-2143-omc-launch-followup' },
            { number: '2149', title: 'fix: resolve global HUD npm package lookup outside Node projects', author: 'aikanedumps', headRefName: 'fix/issue-2148-hud-global-npm' },
            { number: '2151', title: 'feat(hud): make call-count icon rendering configurable', author: 'aikanedumps', headRefName: 'issue-2150-hud-call-count-icons' },
        ];
    ...
    it('assembles a single custom release body with compare link and contributors', () => {
        const body = generateReleaseBody('4.10.2', '# oh-my-caveman v4.10.2: Bug Fixes', ['blue-int', 'DdangJin', 'aikanedumps'], 'v4.10.1');
        expect(body).toContain('npm install -g oh-my-caveman@4.10.2');
        expect(body).toContain('https://github.com/aikanedumps/oh-my-caveman/compare/v4.10.1...v4.10.2');
        expect(body).toContain('@blue-int @DdangJin @aikanedumps');
