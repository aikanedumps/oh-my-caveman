...
describe('Tier-0 contract docs consistency', () => {
    ...
    it('documents all Tier-0 slash commands in REFERENCE.md', () => {
        for (const skillName of ['autopilot', 'ultrawork', 'ralph', 'team', 'ralplan']) {
            expect(referenceDoc).toContain(`/oh-my-caveman:${skillName}`);
        }
    });
    ...
    it('does not contain blank placeholder rows in core skill/command docs', () => {
        ...
        expect(referenceDoc).not.toContain('/oh-my-caveman: <task>');
    ...
    it('does not document removed wrapper slash commands as installed skills', () => {
        expect(referenceDoc).not.toContain('/oh-my-caveman:analyze <target>');
        expect(referenceDoc).not.toContain('/oh-my-caveman:tdd <feature>');
    });
    ...
    it('keeps install and update guidance aligned on canonical setup entrypoints', () => {
        ...
        expect(claudeDoc).toContain('Say "setup omc" or run `/oh-my-caveman:omc-setup`.');
        expect(referenceDoc).toContain('/oh-my-caveman:setup');
    ...
    it('uses the published /docs/ path instead of the removed docs.html path in README links', () => {
        ...
        for (const content of readmes) {
            expect(content).not.toContain('https://yeachan-heo.github.io/oh-my-caveman-website/docs.html');
            expect(content).toContain('https://yeachan-heo.github.io/oh-my-caveman-website/docs/#');
        }
    });
    it('keeps root AGENTS.md aligned with OMC branding and state paths', () => {
        const agentsDoc = readProjectFile('AGENTS.md');
        expect(agentsDoc).toContain('# oh-my-caveman - Intelligent Multi-Agent Orchestration');
        expect(agentsDoc).toContain('You are running with oh-my-caveman (OMC), a multi-agent orchestration layer for Claude Code.');
