...
describe('auto-update reconciliation', () => {
    ...
    it('fetches latest release without Authorization when no GitHub token is configured', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/releases/latest'), {
            headers: {
                ...
                'User-Agent': 'oh-my-caveman-updater',
            },
        });
    });
    ...
    it('syncs active plugin cache roots and logs when copy occurs', () => {
        ...
        const activeRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omc', 'oh-my-caveman', '4.1.5');
        mockedReadFileSync.mockImplementation((path) => {
            ...
            if (normalized.endsWith('/plugins/installed_plugins.json')) {
                return JSON.stringify({
                    plugins: {
                        'oh-my-caveman': [{ installPath: activeRoot }],
                    },
                });
            }
            return '';
    ...
    it('syncs the plugin cache directory when cache root exists', () => {
        const consoleLogSpy = vi.spyOn(console, 'log').mockImplementation(() => { });
        const cacheRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omc', 'oh-my-caveman');
        ...
        mockedReadFileSync.mockImplementation((path) => {
            const normalized = String(path).replace(/\\/g, '/');
            if (normalized === '/usr/lib/node_modules/oh-my-caveman/package.json') {
        ...
        mockedExistsSync.mockImplementation((path) => {
            ...
            if (normalized.startsWith('/usr/lib/node_modules/oh-my-caveman/')) {
        ...
        expect(mockedCpSync).toHaveBeenCalledWith('/usr/lib/node_modules/oh-my-caveman/dist', `${versionedCacheRoot}/dist`, expect.objectContaining({ recursive: true, force: true }));
        expect(mockedCpSync).toHaveBeenCalledWith('/usr/lib/node_modules/oh-my-caveman/package.json', `${versionedCacheRoot}/package.json`, expect.objectContaining({ recursive: true, force: true }));
    ...
    it('only blocks standalone update inside an active plugin session', () => {
        ...
        process.env.CLAUDE_PLUGIN_ROOT = '/tmp/.claude/plugins/cache/omc/oh-my-caveman/4.1.5';
        expect(shouldBlockStandaloneUpdateInCurrentSession()).toBe(false);
    ...
    it('dedupes plugin roots and ignores missing targets during sync', () => {
        const consoleLogSpy = vi.spyOn(console, 'log').mockImplementation(() => { });
        const activeRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omc', 'oh-my-caveman', '4.1.5');
        const staleRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omc', 'oh-my-caveman', '4.1.4');
    ...
    it('allows standalone update when CLAUDE_PLUGIN_ROOT is inherited without an active Claude session', async () => {
        const pluginRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omc', 'oh-my-caveman', '4.1.5');
        const cacheRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omc', 'oh-my-caveman');
        ...
        mockedExecSync.mockImplementation((command) => {
            if (command === 'npm install -g oh-my-caveman@latest') {
                return '';
            }
            if (command === 'npm root -g') {
                return '/usr/lib/node_modules\n';
        ...
        expect(result.success).toBe(true);
        expect(mockedExecSync).toHaveBeenCalledWith('npm install -g oh-my-caveman@latest', expect.any(Object));
    });
    it('runs reconciliation as part of performUpdate without plugin hook reinjection', async () => {
        ...
        process.env.OMC_UPDATE_RECONCILE = '1';
        process.env.CLAUDE_PLUGIN_ROOT = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omc', 'oh-my-caveman', '4.1.5');
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
            ok: true,
            json: async () => ({
                tag_name: 'v4.1.5',
    ...
    it('re-execs with omc.cmd on Windows and persists metadata after reconciliation', async () => {
        ...
        const result = await performUpdate({ verbose: false });
        expect(result.success).toBe(true);
        expect(mockedExecSync).toHaveBeenCalledWith('npm install -g oh-my-caveman@latest', expect.objectContaining({
            windowsHide: true,
        }));
