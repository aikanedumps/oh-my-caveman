...
/**
 * @deprecated Use `tmuxCmd` / `tmuxCmdAsync`; this exists for legacy/internal use only.
 *
 * Executes a tmux command via the system shell (`execSync`). Required when any
 * argument contains `#{` format strings, because MSYS2/Git Bash strips the
 * curly braces from `#{...}` tokens when they appear as `execFile` argv entries.
 * Shell execution passes the string opaque to tmux, preserving the format spec.
 */
 ...
 * @deprecated Use `tmuxCmd` / `tmuxCmdAsync`; this exists for legacy/internal use only.
 *
 * Async variant of `tmuxShell`. See `tmuxShell` for rationale on when shell
 * execution is needed (MSYS2/Git Bash `#{` format string preservation).
 */
...
/**
 * Synchronous tmux command dispatcher — auto-routes between `tmuxShell` and
 * `tmuxExec` based on argument content.
 *
 * When any argument contains `#{`, the command is forwarded to `tmuxShell`
 * (shell execution). This is required on MSYS2/Git Bash (Windows) where
 * `execFileSync` strips the curly braces from `#{...}` tmux format strings,
 * producing silent wrong output. Shell execution passes the string verbatim
 * to tmux, preserving format specs like `#{session_name}`, `#{pane_id}`, etc.
 *
 * When no `#{` is present the safer `tmuxExec` (`execFileSync`) is used,
 * avoiding shell injection risk.
 *
 * @param args - tmux subcommand and arguments, e.g. `['list-sessions', '-F', '#{session_name}']`
 * @param opts - optional execution options forwarded to the underlying call
 * @returns stdout string from the tmux command
 */
export declare function tmuxCmd(args: string[], opts?: TmuxExecOptions & Omit<ExecFileSyncOptionsWithStringEncoding, 'env' | 'encoding'> & {
    encoding?: BufferEncoding;
}): string;
