...
import { buildTmuxShellCommand, buildTmuxShellCommandWithEnv, createHudWatchPane, isClaudeAvailable, killTmuxPane, listHudWatchPaneIdsInCurrentWindow, resolveLaunchPolicy, tmuxCmd, tmuxExec, tmuxSpawn, wrapWithLoginShell, quoteShellArg, sanitizeTmuxToken, } from '../tmux-utils.js';
...
// tmuxCmd routing
// ---------------------------------------------------------------------------
describe('tmuxCmd routing', () => {
    it('routes to tmuxShell (execSync) when any arg contains #{', () => {
        mockedExecFileSync.mockClear();
        mockedExecSync.mockClear();
        mockedExecSync.mockReturnValue('my-session\n');
        const result = tmuxCmd(['display-message', '-p', '#{session_name}']);
        expect(result).toBe('my-session\n');
        expect(mockedExecSync).toHaveBeenCalledWith(expect.stringMatching(/^tmux .+'#\{session_name\}'/), expect.objectContaining({ encoding: 'utf-8' }));
        expect(mockedExecFileSync).not.toHaveBeenCalled();
    });
    it('routes to tmuxExec (execFileSync) when no arg contains #{', () => {
        mockedExecFileSync.mockClear();
        mockedExecSync.mockClear();
        mockedSpawnSync.mockClear();
        // On Linux, resolveTmuxInvocation returns { command: 'tmux', args } directly
        // (no spawnSync needed), so execFileSync is called with 'tmux'.
        mockedExecFileSync.mockReturnValue('');
        tmuxCmd(['kill-session', '-t', 'foo']);
