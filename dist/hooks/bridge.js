...
function isExplicitAskSlashInvocation(promptText) {
    return /^\s*\/(?:oh-my-caveman:)?ask\s+(?:claude|codex|gemini)\b/i.test(promptText);
}
 ...
 * Used to distinguish OMC built-in skills (prefixed with 'oh-my-caveman:')
...
async function processPostToolUse(input) {
    ...
    // (e.g., ralplan consensus handoff into Skill("oh-my-caveman:ralph")).
