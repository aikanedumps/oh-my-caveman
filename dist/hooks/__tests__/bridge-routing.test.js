...
$ ultrawork search the codebase`,
        ...
        it('should activate ralph and linked ultrawork when Skill tool invokes ralph', async () => {
            ...
            try {
                ...
                const input = {
                    sessionId,
                    toolName: 'Skill',
                    toolInput: { skill: 'oh-my-caveman:ralph' },
                    directory: tempDir,
                };
                const result = await processHook('post-tool-use', input);
                expect(result.continue).toBe(true);
        ...
        it('activates ralplan state when Skill tool invokes ralplan directly', async () => {
            ...
            try {
                ...
                const result = await processHook('pre-tool-use', {
                    sessionId,
                    toolName: 'Skill',
                    toolInput: { skill: 'oh-my-caveman:ralplan' },
                    directory: tempDir,
                });
                expect(result.continue).toBe(true);
                const ralplanPath = join(tempDir, '.omc', 'state', 'sessions', sessionId, 'ralplan-state.json');
        ...
        it('arms ralplan startup state and init context for explicit /ralplan invoke in UserPromptSubmit', async () => {
            ...
            try {
                ...
                const result = await processHook('keyword-detector', {
                    sessionId,
                    prompt: '/oh-my-caveman:ralplan issue #2622',
                    directory: tempDir,
                });
                expect(result.continue).toBe(true);
                ...
                expect(hookSpecificOutput.additionalContext).toContain('/oh-my-caveman:ralplan issue #2622');
                const ralplanPath = join(tempDir, '.omc', 'state', 'sessions', sessionId, 'ralplan-state.json');
                expect(existsSync(ralplanPath)).toBe(true);
                const ralplanState = JSON.parse(readFileSync(ralplanPath, 'utf-8'));
                expect(ralplanState.active).toBe(true);
        ...
        it('seeds workflow slot for explicit /deep-interview slash invocation in UserPromptSubmit', async () => {
            ...
            try {
                ...
                const result = await processHook('keyword-detector', {
                    sessionId,
                    prompt: '/oh-my-caveman:deep-interview explore auth flows',
                    directory: tempDir,
                });
                expect(result.continue).toBe(true);
        ...
        it('seeds workflow slot when Skill tool invokes oh-my-caveman:deep-interview', async () => {
            ...
            try {
                ...
                const result = await processHook('pre-tool-use', {
                    sessionId,
                    toolName: 'Skill',
                    toolInput: { skill: 'oh-my-caveman:deep-interview' },
                    directory: tempDir,
                });
                expect(result.continue).toBe(true);
                const slotPath = join(tempDir, '.omc', 'state', 'sessions', sessionId, 'skill-active-state.json');
        ...
        it('seeds workflow slot when Skill tool invokes oh-my-caveman:self-improve', async () => {
            ...
            try {
                ...
                const result = await processHook('pre-tool-use', {
                    sessionId,
                    toolName: 'Skill',
                    toolInput: { skill: 'oh-my-caveman:self-improve' },
                    directory: tempDir,
                });
                expect(result.continue).toBe(true);
                const slotPath = join(tempDir, '.omc', 'state', 'sessions', sessionId, 'skill-active-state.json');
