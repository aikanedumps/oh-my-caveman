 ...
 * 2. Active ralph loop (until cancelled via /oh-my-caveman:cancel)
...
import { readRalphState, writeRalphState, incrementRalphIteration, RALPH_LOCK_BUSY, clearRalphState, findPrdPath, getPrdCompletionStatus, getRalphContext, getStory, markStoryIncomplete, markStoryArchitectVerified, readVerificationState, startVerification, recordArchitectFeedback, getArchitectVerificationPrompt, getArchitectRejectionContinuationPrompt, detectArchitectApproval, detectArchitectRejection, clearVerificationState, } from '../ralph/index.js';
...
async function checkRalphLoop(sessionId, directory, cancelInProgress) {
    ...
    if (hardMax > 0 && state.iteration >= hardMax) {
        ...
        return {
            shouldBlock: true,
            message: `[RALPH - HARD LIMIT] Reached hard max iterations (${hardMax}). Mode auto-disabled. Restart with /oh-my-caveman:ralph if needed.`,
            mode: 'ralph',
    ...
    // Retry once on transient lock contention (synchronous spin so this function stays sync).
    const rawIncrement = incrementRalphIteration(workingDir, sessionId);
    let newState;
    if (rawIncrement === RALPH_LOCK_BUSY) {
        const retryDeadline = Date.now() + 100;
        ...
        const retryResult = incrementRalphIteration(workingDir, sessionId);
        if (retryResult === RALPH_LOCK_BUSY) {
            console.warn("[ralph] incrementRalphIteration: lock still busy after retry; deferring continuation to next stop-hook invocation.");
            return null;
        }
        newState = retryResult;
    }
    else {
        newState = rawIncrement;
    }
    if (!newState) {
        return null;
    }
...
4. When FULLY complete (after ${state.critic_mode === 'codex' ? 'Codex critic' : state.critic_mode === 'critic' ? 'Critic' : 'Architect'} verification), run \`/oh-my-caveman:cancel\` to cleanly exit and clean up state files. If cancel fails, retry with \`/oh-my-caveman:cancel --force\`.
...
When done, run \`/oh-my-caveman:cancel\` to cleanly exit.
...
async function checkUltrawork(sessionId, directory, _hasIncompleteTodos, cancelInProgress) {
    ...
    if (hardMax > 0 && state.reinforcement_count >= hardMax) {
        ...
        return {
            shouldBlock: true,
            message: '[ULTRAWORK - HARD LIMIT] Reached hard max iterations (' + hardMax + '). Mode auto-disabled. Restart with /oh-my-caveman:ultrawork if needed.',
            mode: 'ultrawork',
...
export async function checkPersistentModes(sessionId, directory, stopContext // NEW: from todo-continuation types
    ...
    // Fix for: https://github.com/aikanedumps/oh-my-caveman/issues/777
