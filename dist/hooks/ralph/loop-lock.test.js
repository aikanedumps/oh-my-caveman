...
function writeRalphStateFile(dir, sessionId, iteration) {
...
function readIteration(dir, sessionId) {
    const stateFile = join(dir, ".omc", "state", "sessions", sessionId, "ralph-state.json");
...
const workerScript = join(projectRoot, "dist", "hooks", "ralph", "loop-lock-worker.js");
// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------
describe("incrementRalphIteration — file-lock (US-103)", () => {
    ...
    it("returns RALPH_LOCK_BUSY and logs a warning when lock cannot be acquired", async () => {
        const { incrementRalphIteration, RALPH_LOCK_BUSY } = await import("./loop.js");
    ...
    it("serialises 5 concurrent increments — no lost updates", () => {
        ...
        const procs = Array.from({ length: CONCURRENCY }, (_, i) => spawnSync("node", [workerScript, dir, sessionId, String(i * 10)], {
            timeout: 10_000,
            encoding: "utf-8",
        }));
        for (const proc of procs) {
            if (proc.status !== 0) {
                errors.push(proc.stderr?.trim() || proc.stdout?.trim() || "non-zero exit");
            }
        }
        try {
            if (errors.length > 0) {
...
//# sourceMappingURL=loop-lock.test.js.map