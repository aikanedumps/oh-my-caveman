 ...
 */
export const RALPH_LOCK_BUSY = Symbol("RALPH_LOCK_BUSY");
...
export function incrementRalphIteration(directory, sessionId) {
    ...
    if (!handle) {
        console.warn(`[ralph] incrementRalphIteration: could not acquire file lock within 500ms (${lockPath}). Signalling lock-busy for retry.`);
...
export function createRalphLoopHook(directory) {
    const startLoop = (sessionId, prompt, options) => {
        ...
        if (isUltraQAActive(directory, sessionId)) {
            console.error("Cannot start Ralph Loop while UltraQA is active. Cancel UltraQA first with /oh-my-caveman:cancel.");
            return false;
        }
