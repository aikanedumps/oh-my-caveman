...
describe('skill-state', () => {
    ...
    describe('getSkillProtection', () => {
        ...
        it('strips oh-my-caveman: prefix', () => {
            expect(getSkillProtection('oh-my-caveman:plan')).toBe('medium');
            expect(getSkillProtection('oh-my-caveman:ralph')).toBe('none');
        });
        ...
        it('returns none for project custom skills with same name as OMC skills (issue #1581)', () => {
            // rawSkillName without oh-my-caveman: prefix → project custom skill
        ...
        it('returns protection for OMC skills when rawSkillName has prefix', () => {
            expect(getSkillProtection('plan', 'oh-my-caveman:plan')).toBe('medium');
            expect(getSkillProtection('deepinit', 'oh-my-caveman:deepinit')).toBe('heavy');
        });
    ...
    describe('writeSkillActiveState', () => {
        ...
        it('strips namespace prefix from skill name', () => {
            const state = writeSkillActiveState(tempDir, 'oh-my-caveman:plan', 'session-1');
            expect(state.skill_name).toBe('plan');
        });
        ...
        it('writes state for OMC skills when rawSkillName has prefix', () => {
            const state = writeSkillActiveState(tempDir, 'plan', 'session-1', 'oh-my-caveman:plan');
            expect(state).not.toBeNull();
            expect(state.skill_name).toBe('plan');
            expect(state.max_reinforcements).toBe(5);
        });
    ...
    describe('upsertWorkflowSkillSlot — pure helper', () => {
        ...
        it('strips oh-my-caveman: prefix from skill name', () => {
            const state = upsertWorkflowSkillSlot(emptySkillActiveStateV2(), 'oh-my-caveman:ralph', {
                session_id: 's1',
                mode_state_path: 'r.json',
                initialized_mode: 'ralph',
                initialized_state_path: '',
            ...
            expect(state.active_skills['oh-my-caveman:ralph']).toBeUndefined();
        });
